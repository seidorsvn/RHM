//
//  DetalleAutoresViewController.m
//  RHM
//
...
@implementation DetalleAutoresViewController

@synthesize masterPopoverController, tvDetalleAutores, pcContenidoMultimedia, aItems, iPosicion;
...
- (void)viewDidLoad
{
    ...
    [self.navigationItem setRightBarButtonItems:[[NSArray alloc] initWithObjects:[[UIBarButtonItem alloc] initWithTitle:@"Me gusta leer" style:UIBarButtonItemStyleBordered target:self action:@selector(doMeGustaLeer)], nil]];
}
...
{
    UITableViewCell *tvcDetalleAutores = [tableView dequeueReusableCellWithIdentifier:[NSString stringWithFormat:@"ciS%dR%d", [indexPath section], [indexPath row]]];
    if (tvcDetalleAutores == nil) {
        tvcDetalleAutores = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:[NSString stringWithFormat:@"ciS%dR%d", [indexPath section], [indexPath row]]];
    }
    for (UIView *view in [[[tvcDetalleAutores subviews]objectAtIndex:0] subviews]) {
...
- (void)doTapBotonContenidoMultimedia:(id)sender
{
    if ([sender state] == UIGestureRecognizerStateBegan) {
        ...
        NSData *dImagen = [NSData dataWithContentsOfURL:[NSURL URLWithString:@"http://noticias.terra.es/genteycultura/2009/0520/fotos-media/ken-follett-y-su-banda-de-blues-actuaran-en-madrid.jpg"]];
        NSString *sPath = [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) objectAtIndex:0] stringByAppendingPathComponent:@"/KenFollett.jpg"];
...
{
    [self performSegueWithIdentifier:@"ListadoArtículo" sender:sender];
}

- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    if ([[segue identifier] isEqualToString:@"ListadoArtículo"]) {
