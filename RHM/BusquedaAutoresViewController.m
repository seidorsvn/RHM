//
//  BusquedaAutoresViewController.m
//  RHM
//
...
#import "BusquedaAutoresViewController.h"
#import "InicioViewController.h"
#import "ListadoAutoresViewController.h"
#import "AppDelegate.h"

@implementation BusquedaAutoresViewController

@synthesize masterPopoverController, tvBusquedaAutores, tvBuscarMatchCode, pcBuscarMatchCode, iBuscarMatchCodeTag, tfCampoActivo, aItems;
...
{
    [tvBusquedaAutores scrollToRowAtIndexPath:[self.tvBusquedaAutores indexPathForCell:(UITableViewCell *)[[textField superview] superview]] atScrollPosition:UITableViewScrollPositionMiddle animated:YES];
...
- (void)keyboardWillShow:(NSNotification *)note
{
    ...
    if (tfCampoActivo)
    {
        CGRect textFieldRect = [self.tvBusquedaAutores convertRect:[tfCampoActivo bounds] fromView:tfCampoActivo];
        [self.tvBusquedaAutores scrollRectToVisible:textFieldRect animated:NO];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    while (!bEncontrado) {
        i++;
        if ([[[[tvBusquedaAutores subviews] objectAtIndex:i] subviews] count]>1 && [[[[[[tvBusquedaAutores subviews] objectAtIndex:i] subviews] objectAtIndex:1] subviews] count]>2 && [[[[[[[tvBusquedaAutores subviews] objectAtIndex:i] subviews] objectAtIndex:1] subviews] objectAtIndex:2] isKindOfClass:[UIButton class]] && [[[[[[[tvBusquedaAutores subviews] objectAtIndex:i] subviews] objectAtIndex:1] subviews] objectAtIndex:2] tag] == iBuscarMatchCodeTag) {
            bEncontrado = YES;
        }
    }
    UITextField *tfBuscarMatchCode = [[[[[[tvBusquedaAutores subviews] objectAtIndex:i] subviews] objectAtIndex:1] subviews] objectAtIndex:1];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    switch ([tableView tag]) {
        case 0:
        {
            UITableViewCell *tvcBusquedaAutores = [tableView dequeueReusableCellWithIdentifier:[NSString stringWithFormat:@"ciS%dR%d", [indexPath section], [indexPath row]]];
            if (tvcBusquedaAutores == nil) {
                tvcBusquedaAutores = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:[NSString stringWithFormat:@"ciS%dR%d", [indexPath section], [indexPath row]]];
            }
            if ([[[[tvcBusquedaAutores subviews] lastObject] subviews] count]>1) {
                [[[[[tvcBusquedaAutores subviews] lastObject] subviews] objectAtIndex:1] setDelegate:self];
            }
            return tvcBusquedaAutores;
            break;
        }
        case 1:
        {
...
- (IBAction)doBuscarMatchCode:(id)sender
{
    ...
    while (!bEncontrado) {
        i++;
        if ([[[[tvBusquedaAutores subviews] objectAtIndex:i] subviews] count]>1 && [[[[[[tvBusquedaAutores subviews] objectAtIndex:i] subviews] objectAtIndex:1] subviews] count]>2 && [[[[[[[tvBusquedaAutores subviews] objectAtIndex:i] subviews] objectAtIndex:1] subviews] objectAtIndex:2] isKindOfClass:[UIButton class]] && [[[[[[[tvBusquedaAutores subviews] objectAtIndex:i] subviews] objectAtIndex:1] subviews] objectAtIndex:2] tag] == [sender tag]) {
            bEncontrado = YES;
        }
    }
    UIButton *buDisclosure = [[[[[[tvBusquedaAutores subviews] objectAtIndex:i] subviews] objectAtIndex:1] subviews] objectAtIndex:2];
    [sbBuscarMatchCode setText:[[NSString stringWithFormat:@"%@", [(UITextField *)[[[[[[tvBusquedaAutores subviews] objectAtIndex:i] subviews] objectAtIndex:1] subviews] objectAtIndex:1] text]]stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]]];
    ...
    [sbBuscarMatchCode setPlaceholder:[[[[[[[[tvBusquedaAutores subviews] objectAtIndex:i] subviews] objectAtIndex:1] subviews] objectAtIndex:1] placeholder] stringByReplacingOccurrencesOfString:@"Establecer " withString:@"Filtrar "]];
...
{
    [self performSegueWithIdentifier:@"ListadoAutores" sender:sender];
}

- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    if ([[segue identifier] isEqualToString:@"ListadoAutores"]) {
        ListadoAutoresViewController *lavcListadoAutores = [segue destinationViewController];
        NSMutableArray *aAuxiliar = [[NSMutableArray alloc] init];
        for (int i=0; i<50; i++) {
            [aAuxiliar addObject:[NSString stringWithFormat:@"Autor %d", i]];
        }
        [lavcListadoAutores setAItems:aAuxiliar];
    }
}
