//
//  BusquedaClientesViewController.m
//  RHM
//
...
#import "BusquedaClientesViewController.h"
#import "InicioViewController.h"
#import "ListadoClientesViewController.h"
#import "AppDelegate.h"

@implementation BusquedaClientesViewController

@synthesize masterPopoverController, tvBusquedaClientes, tvBuscarMatchCode, scBloqueoEntrega, pcBuscarMatchCode, iBuscarMatchCodeTag, tfCampoActivo, aItems;
...
- (void)viewDidUnload
{
    ...
    [self setTvBusquedaClientes:nil];
    [self setTfCampoActivo:nil];
    [self setAItems:nil];
    [self setScBloqueoEntrega:nil];
    [super viewDidUnload];
}
...
{
    [tvBusquedaClientes scrollToRowAtIndexPath:[self.tvBusquedaClientes indexPathForCell:(UITableViewCell *)[[textField superview] superview]] atScrollPosition:UITableViewScrollPositionMiddle animated:YES];
...
- (void)keyboardWillShow:(NSNotification *)note
{
    ...
    if (tfCampoActivo)
    {
        CGRect textFieldRect = [self.tvBusquedaClientes convertRect:[tfCampoActivo bounds] fromView:tfCampoActivo];
        [self.tvBusquedaClientes scrollRectToVisible:textFieldRect animated:NO];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    while (!bEncontrado) {
        i++;
        if ([[[[tvBusquedaClientes subviews] objectAtIndex:i] subviews] count]>1 && [[[[[[tvBusquedaClientes subviews] objectAtIndex:i] subviews] objectAtIndex:1] subviews] count]>2 && [[[[[[[tvBusquedaClientes subviews] objectAtIndex:i] subviews] objectAtIndex:1] subviews] objectAtIndex:2] isKindOfClass:[UIButton class]] && [[[[[[[tvBusquedaClientes subviews] objectAtIndex:i] subviews] objectAtIndex:1] subviews] objectAtIndex:2] tag] == iBuscarMatchCodeTag) {
            bEncontrado = YES;
        }
    }
    UITextField *tfBuscarMatchCode = [[[[[[tvBusquedaClientes subviews] objectAtIndex:i] subviews] objectAtIndex:1] subviews] objectAtIndex:1];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    switch ([tableView tag]) {
        case 0:
        {
            UITableViewCell *tvcBusquedaClientes = [tableView dequeueReusableCellWithIdentifier:[NSString stringWithFormat:@"ciS%dR%d", [indexPath section], [indexPath row]]];
            if (tvcBusquedaClientes == nil) {
                tvcBusquedaClientes = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:[NSString stringWithFormat:@"ciS%dR%d", [indexPath section], [indexPath row]]];
            }
            if ([[[[tvcBusquedaClientes subviews] lastObject] subviews] count]>1) {
                [[[[[tvcBusquedaClientes subviews] lastObject] subviews] objectAtIndex:1] setDelegate:self];
            }
            return tvcBusquedaClientes;
            break;
        }
        case 1:
        {
...
- (IBAction)doBuscarMatchCode:(id)sender
{
    ...
    while (!bEncontrado) {
        i++;
        if ([[[[tvBusquedaClientes subviews] objectAtIndex:i] subviews] count]>1 && [[[[[[tvBusquedaClientes subviews] objectAtIndex:i] subviews] objectAtIndex:1] subviews] count]>2 && [[[[[[[tvBusquedaClientes subviews] objectAtIndex:i] subviews] objectAtIndex:1] subviews] objectAtIndex:2] isKindOfClass:[UIButton class]] && [[[[[[[tvBusquedaClientes subviews] objectAtIndex:i] subviews] objectAtIndex:1] subviews] objectAtIndex:2] tag] == [sender tag]) {
            bEncontrado = YES;
        }
    }
    UIButton *buDisclosure = [[[[[[tvBusquedaClientes subviews] objectAtIndex:i] subviews] objectAtIndex:1] subviews] objectAtIndex:2];
    [sbBuscarMatchCode setText:[[NSString stringWithFormat:@"%@", [(UITextField *)[[[[[[tvBusquedaClientes subviews] objectAtIndex:i] subviews] objectAtIndex:1] subviews] objectAtIndex:1] text]]stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]]];
    ...
    [sbBuscarMatchCode setPlaceholder:[[[[[[[[tvBusquedaClientes subviews] objectAtIndex:i] subviews] objectAtIndex:1] subviews] objectAtIndex:1] placeholder] stringByReplacingOccurrencesOfString:@"Establecer " withString:@"Filtrar "]];
...
{
    [self performSegueWithIdentifier:@"ListadoClientes" sender:sender];
}

- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    if ([[segue identifier] isEqualToString:@"ListadoClientes"]) {
        ListadoClientesViewController *lcvcListadoClientes = [segue destinationViewController];
        NSMutableArray *aAuxiliar = [[NSMutableArray alloc] init];
        for (int i=0; i<50; i++) {
            [aAuxiliar addObject:[NSString stringWithFormat:@"Cliente %d", i]];
        }
        [lcvcListadoClientes setAItems:aAuxiliar];
    }
}
