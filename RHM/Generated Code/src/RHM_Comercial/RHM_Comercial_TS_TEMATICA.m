...
@interface RHM_Comercial_TS_TEMATICA(private)
- (void)bind:(id<SUPResultSetWrapper>)resultSet;
- (void)bindWithQuery:(id<SUPResultSetWrapper>)resultSet:(SUPQuery*)query;
+ (RHM_Comercial_TS_TEMATICA*)find:(int64_t)id inTable:(SUPEntityTable)table;
+ (void)importData:(RHM_Comercial_TS_TEMATICA*)serverEntity:(SUPJsonObject*)jsonObject;
- (void)merge:(RHM_Comercial_TS_TEMATICA*)entity withJson:(SUPJsonObject*)json;
- (void)setAllAttributesFrom:(RHM_Comercial_TS_TEMATICA*)other;
@end
@implementation RHM_Comercial_TS_TEMATICA

@synthesize BUKRS = _BUKRS;
@synthesize ZCODTEM = _ZCODTEM;
@synthesize ZDESCR = _ZDESCR;
@synthesize ZDESBR = _ZDESBR;
...
- (void)setZCODTEM:(NSString*)newZCODTEM
{
    if (newZCODTEM != self->_ZCODTEM)
    {
		[self->_ZCODTEM release];
        self->_ZCODTEM = [newZCODTEM retain];
        self->_isDirty = YES;
    }
}
...
- (void)setZDESBR:(NSString*)newZDESBR
{
    if (newZDESBR != self->_ZDESBR)
    {
		[self->_ZDESBR release];
        self->_ZDESBR = [newZDESBR retain];
        self->_isDirty = YES;
    }
}
...
- (void)readJson:(SUPJsonObject*)json:(int32_t)flags
{
        if ([json containsKey:@"BUKRS"])
    {
        ...
        if ([json containsKey:@"ZCODTEM"])
    {
        self.ZCODTEM = [json getString:@"ZCODTEM"];
    }
        if ([json containsKey:@"ZDESCR"])
    {
        self.ZDESCR = [json getString:@"ZDESCR"];
    }
        if ([json containsKey:@"ZDESBR"])
    {
        self.ZDESBR = [json getString:@"ZDESBR"];
    }
...
- (SUPJsonObject*)json:(int32_t)flags
{
	...
	if (all)
	{
	    ...
	    [newJson setString:@"ZCODTEM":self.ZCODTEM];
	    [newJson setString:@"ZDESCR":self.ZDESCR];
	    [newJson setString:@"ZDESBR":self.ZDESBR];
	    [newJson setBoolean:@"disableSubmit":self.disableSubmit];
	}
    [newJson setString:@"?":@"TS_TEMATICA"];
    [newJson autorelease];
    return newJson;
}
+ (SUPObjectList*)fromJSONList:(SUPJsonArray*)jsonArray
{
	...
	{
	    RHM_Comercial_TS_TEMATICA* inst = [[RHM_Comercial_TS_TEMATICA alloc] initFromJson:jsonObject:0];
...
+ (SUPJsonArray*)toJSONList:(SUPObjectList*)instanceList
{
	...
	for (RHM_Comercial_TS_TEMATICA* inst in instanceList)
	{
...
- (void)bind:(id<SUPResultSetWrapper>)resultSet
{
    @try
    {
        ...
        self.ZCODTEM = [resultSet getString:2:@"b"];
        self.ZDESCR = [resultSet getString:3:@"c"];
        self.ZDESBR = [resultSet getString:4:@"d"];
...
- (void)bindWithQuery:(id<SUPResultSetWrapper>)resultSet:(SUPQuery*)query
{
    ...
    SUPEntityMetaData *entity = [RHM_Comercial_TS_TEMATICA metaData];
    ...
    for(int j=0;j<[cl size];j++)
    {
		...
		if([amd.name isEqualToString:@"ZCODTEM"])
        	self.ZCODTEM = [resultSet getString:j+1:@"b"];
		if([amd.name isEqualToString:@"ZDESCR"])
        	self.ZDESCR = [resultSet getString:j+1:@"c"];
		if([amd.name isEqualToString:@"ZDESBR"])
        	self.ZDESBR = [resultSet getString:j+1:@"d"];
...
- (void)copyAll:(RHM_Comercial_TS_TEMATICA*)other
{
	[self copyAll:other withParent:(SUPAbstractEntity*)other];
}


- (void)copyAll:(RHM_Comercial_TS_TEMATICA*)other withParent:(SUPAbstractEntity*)parent
{
    [self setBUKRS:other.BUKRS];
    
    [self setZCODTEM:other.ZCODTEM];
    
    [self setZDESCR:other.ZDESCR];
    
    [self setZDESBR:other.ZDESBR];
...
id<SUPResultSetWrapper> resultSet = nil;
    {
        ...
        {    
            ...
            statement = [SUPStatementBuilder prepareQuery:connection:[RHM_Comercial_RHM_ComercialDB metaData]:[RHM_Comercial_TS_TEMATICA metaData]:query];
...
+ (RHM_Comercial_TS_TEMATICA*)find_by_id_string:(NSString*)id_string
{
    if(id_string == nil) return nil;
	int64_t key;
	NSNumberFormatter *nf = [[[NSNumberFormatter alloc] init] autorelease];
	key = [[nf numberFromString:id_string] longValue];
    return [RHM_Comercial_TS_TEMATICA find:key];
	
}

+ (RHM_Comercial_TS_TEMATICA*)find:(int64_t)id
{
	return [self find:id inTable:MainEntityTable];
}

+ (RHM_Comercial_TS_TEMATICA*)find:(int64_t)id_ inTable:(SUPEntityTable)table
{
    RHM_Comercial_TS_TEMATICA* result = nil;
	RHM_Comercial_TS_TEMATICA* downloadState = nil;
    ...
    @try
    {
    	...
    	//@synchronized(connection)
    	{
        	@try
        	{
            	...
            	{
                	preparedStatement = [connection prepareStatement:@"select \"a\",\"b\",\"c\",\"d\",\"_pf\",\"_pc\",\"_rp\",\"_rf\",\"e\",\"_rc\",\"_ds\" from rhm_comercial_1_0_ts_tematica where \"e\"=? order by \"_pf\""];
            	}
            	else
            	{
                	preparedStatement = [connection prepareStatement:@"select \"a\",\"b\",\"c\",\"d\",\"_pf\",\"_pc\",\"_rp\",\"_rf\",\"e\",\"_rc\",\"_ds\" from rhm_comercial_1_0_ts_tematica_os where \"e\"=? order by \"_pf\""];
            	}
            	...
            	while ([resultSet next])
            	{
                	...
                	result = [[RHM_Comercial_TS_TEMATICA alloc] init];
                	...
                	{
 	              		downloadState = [RHM_Comercial_TS_TEMATICA getInstance];
...
id<SUPResultSetWrapper> resultSet = nil;
    {
        ...
        {    
            ...
            connection = [RHM_Comercial_RHM_ComercialDB getConnectionWrapper];
            int32_t count = 0;
            RHM_Comercial_TS_TEMATICAList* list = [[RHM_Comercial_TS_TEMATICAList alloc] init];
            ...
            {
                count++;   
                RHM_Comercial_TS_TEMATICA* entity = [[RHM_Comercial_TS_TEMATICA alloc] init];
...
- (id<SUPStatementWrapper>)getCreateStatement:(id<SUPConnectionWrapper>)connection forTable:(SUPEntityTable)table;
{
    ...
    if (table == MainEntityTable)
    {
        preparedStatement = [connection prepareStatement:@"insert into rhm_comercial_1_0_ts_tematica (\"a\",\"b\",\"c\",\"d\",\"_pf\",\"_pc\",\"_rp\",\"_rf\",\"e\",\"_rc\",\"_ds\") values (?,?,?,?,?,?,?,?,?,?,?)"];
    }
    else
    {
        preparedStatement = [connection prepareStatement:@"insert into rhm_comercial_1_0_ts_tematica_os (\"a\",\"b\",\"c\",\"d\",\"_pf\",\"_pc\",\"_rp\",\"_rf\",\"e\",\"_rc\",\"_ds\") values (?,?,?,?,?,?,?,?,?,?,?)"];
    }
    // Add updated values
	if(self.BUKRS == nil)
		self.BUKRS = @"";
		//RHM_Comercial_TS_TEMATICA getCreateStatement:BUKRS not allowed to be null
    [preparedStatement setString:1:@"a":self.BUKRS];
	if(self.ZCODTEM == nil)
		self.ZCODTEM = @"";
		//RHM_Comercial_TS_TEMATICA getCreateStatement:ZCODTEM not allowed to be null
    [preparedStatement setString:2:@"b":self.ZCODTEM];
	if(self.ZDESCR == nil)
		self.ZDESCR = @"";
		//RHM_Comercial_TS_TEMATICA getCreateStatement:ZDESCR not allowed to be null
    [preparedStatement setString:3:@"c":self.ZDESCR];
	if(self.ZDESBR == nil)
		self.ZDESBR = @"";
		//RHM_Comercial_TS_TEMATICA getCreateStatement:ZDESBR not allowed to be null
    [preparedStatement setString:4:@"d":self.ZDESBR];
...
- (id<SUPStatementWrapper>)getDeleteStatement:(id<SUPConnectionWrapper>)connection forTable:(SUPEntityTable)table;
{
    ...
    if (table == MainEntityTable)
    {
        preparedStatement = [connection prepareStatement:@"delete from rhm_comercial_1_0_ts_tematica where \"_pf\"=? and \"e\"=?"];
    }
    else
    {
        preparedStatement = [connection prepareStatement:@"delete from rhm_comercial_1_0_ts_tematica_os where \"_pf\"=? and \"e\"=?"];
    }
...
- (id<SUPStatementWrapper>)getUpdateStatement:(id<SUPConnectionWrapper>)connection forTable:(SUPEntityTable)table;
{
    ...
    if (table == MainEntityTable)
    {
    	preparedStatement = [connection prepareStatement:@"update rhm_comercial_1_0_ts_tematica set \"a\"=?,\"b\"=?,\"c\"=?,\"d\"=?,\"_pc\"=?,\"_rp\"=?,\"_rf\"=?,\"_rc\"=?,\"_ds\"=? where \"_pf\"=? and \"e\"=?"];
    }
    else
    {
        preparedStatement = [connection prepareStatement:@"update rhm_comercial_1_0_ts_tematica_os set \"a\"=?,\"b\"=?,\"c\"=?,\"d\"=?,\"_pc\"=?,\"_rp\"=?,\"_rf\"=?,\"_rc\"=?,\"_ds\"=? where \"_pf\"=? and \"e\"=?"];
    }
    // Add updated values
    int parameterIndex = 0;
	if(self.BUKRS == nil)
		self.BUKRS = @"";
		//RHM_Comercial_TS_TEMATICA getUpdateStatement:BUKRS not allowed to be null
    parameterIndex++;
    [preparedStatement setString:parameterIndex:@"a":self.BUKRS];
	if(self.ZCODTEM == nil)
		self.ZCODTEM = @"";
		//RHM_Comercial_TS_TEMATICA getUpdateStatement:ZCODTEM not allowed to be null
    parameterIndex++;
    [preparedStatement setString:parameterIndex:@"b":self.ZCODTEM];
	if(self.ZDESCR == nil)
		self.ZDESCR = @"";
		//RHM_Comercial_TS_TEMATICA getUpdateStatement:ZDESCR not allowed to be null
    parameterIndex++;
    [preparedStatement setString:parameterIndex:@"c":self.ZDESCR];
	if(self.ZDESBR == nil)
		self.ZDESBR = @"";
		//RHM_Comercial_TS_TEMATICA getUpdateStatement:ZDESBR not allowed to be null
    parameterIndex++;
    [preparedStatement setString:parameterIndex:@"d":self.ZDESBR];
    parameterIndex++;
...
+ (void)importData:(RHM_Comercial_TS_TEMATICA*)serverEntity:(SUPJsonObject*)jsonObject
{
	...
	RHM_Comercial_TS_TEMATICA* serverEntity2 = [RHM_Comercial_TS_TEMATICA getInstance];
    ...
    if([RHM_Comercial_TS_TEMATICA callbackHandler])
		[[RHM_Comercial_TS_TEMATICA callbackHandler] beforeImport:serverEntity2];
    ...
    } else if(delete)
    {
        ...
        if(isCreated) 
        {
            RHM_Comercial_TS_TEMATICA *clientEntity = [RHM_Comercial_TS_TEMATICA find:[serverEntity _pk]];
	...
	if(update)
    {
    	...
    	if(clientEntity.pending)
    		clientEntity = (RHM_Comercial_TS_TEMATICA*)clientEntity.downloadState;
        @try 
        {
    ...
    if([RHM_Comercial_TS_TEMATICA callbackHandler])
		[[RHM_Comercial_TS_TEMATICA callbackHandler] onImport:serverEntity];
...
+ (RHM_Comercial_TS_TEMATICA*)load:(int64_t)id
{
	RHM_Comercial_TS_TEMATICA* entity = [self find:id inTable:MainEntityTable];
	//RHM_Comercial_TS_TEMATICA* entity = [self find:id];
    if (entity == nil)
    {
		@throw [SUPObjectNotFoundException withMessage:@"-load: in RHM_Comercial_TS_TEMATICA: entity not found in database"];
    }
    return entity;
}

static SUPEntityMetaData* RHM_Comercial_TS_TEMATICA_META_DATA;
...
+ (SUPEntityMetaData*)metaData
{
    if (! (_valid_))
    {
       RHM_Comercial_TS_TEMATICA_META_DATA = [SUPEntityMetaData getInstance];
       RHM_Comercial_TS_TEMATICA_META_DATA.id = _ENTITY_NUMBER_FOR_RHM_Comercial_TS_TEMATICA_;
       RHM_Comercial_TS_TEMATICA_META_DATA.name = @"TS_TEMATICA";
       [RHM_Comercial_TS_TEMATICA_META_DATA setClass:[RHM_Comercial_TS_TEMATICA class]];

	   RHM_Comercial_TS_TEMATICA_META_DATA.isClientOnly = NO;
       ...
       // Handle attributes
		...
		[a_BUKRS setScale:0];
		[a_BUKRS setIsKey:NO];
		SUPAttributeMetaData_DC* a_ZCODTEM = [RHM_Comercial_RHM_ComercialDB createAttributeMetaData:2:(SUPString)@"ZCODTEM":[SUPDataType forName:@"string"]];
		[a_ZCODTEM setColumn:@"\"b\""];
		[a_ZCODTEM setMaxLength:6];
		[a_ZCODTEM setPrecision:0];
		[a_ZCODTEM setScale:0];
		[a_ZCODTEM setIsKey:NO];
		...
		SUPAttributeMetaData_DC* a_ZDESBR = [RHM_Comercial_RHM_ComercialDB createAttributeMetaData:4:(SUPString)@"ZDESBR":[SUPDataType forName:@"string"]];
		[a_ZDESBR setColumn:@"\"d\""];
		[a_ZDESBR setMaxLength:15];
		[a_ZDESBR setPrecision:0];
		[a_ZDESBR setScale:0];
		[a_ZDESBR setIsKey:NO];
		...
		[a_disableSubmit setIsKey:NO];
 		...
 		[attributes addThis:a_ZCODTEM];
 		[attributes addThis:a_ZDESCR];
 		[attributes addThis:a_ZDESBR];
 		...
 		RHM_Comercial_TS_TEMATICA_META_DATA.attributes = attributes;
 		
 		SUPAttributeMap * attributeMap = [SUPAttributeMap getInstance];
    	[attributeMap setAttributes:attributes];
    	RHM_Comercial_TS_TEMATICA_META_DATA.attributeMap = [attributeMap finishInit];
 
 		// Handle operations
 		SUPOperationMetaData* o_findAll_0 = [RHM_Comercial_RHM_ComercialDB createOperationMetaData:1:(SUPString)@"findAll":[SUPDataType forName:@"TS_TEMATICA*"]:true];
		...
		[o_findAll_0 setIsDelete:NO]; 		
 		...
 		SUPOperationMetaData* o_getPendingObjects_0 = [RHM_Comercial_RHM_ComercialDB createOperationMetaData:2:(SUPString)@"getPendingObjects":[SUPDataType forName:@"TS_TEMATICA*"]:true];
		...
		[o_getPendingObjects_0 setIsDelete:NO]; 		
 		...
 		SUPOperationMetaData* o_getPendingObjects_1 = [RHM_Comercial_RHM_ComercialDB createOperationMetaData:3:(SUPString)@"getPendingObjects":[SUPDataType forName:@"TS_TEMATICA*"]:true];
  		{
		...
		[o_getPendingObjects_1 setIsDelete:NO]; 		
 		...
 		SUPOperationMetaData* o_getReplayPendingObjects_0 = [RHM_Comercial_RHM_ComercialDB createOperationMetaData:4:(SUPString)@"getReplayPendingObjects":[SUPDataType forName:@"TS_TEMATICA*"]:true];
		...
		[o_save_0 setIsDelete:NO]; 		
 		[o_save_0 finishInit];
 		SUPOperationMetaData* o_findWithQuery_1 = [RHM_Comercial_RHM_ComercialDB createOperationMetaData:12:(SUPString)@"findWithQuery":[SUPDataType forName:@"TS_TEMATICA*"]:true];
  		{
 			SUPObjectList *parameters_list = nil;
		...
		[o_countReplayPendingObjects_0 setIsDelete:NO]; 		
 		...
 		RHM_Comercial_TS_TEMATICA_META_DATA.operations = operations;
 		
		SUPOperationMap *operationMap = [SUPOperationMap getInstance];
		[operationMap setOperations:operations];
		RHM_Comercial_TS_TEMATICA_META_DATA.operationMap = [operationMap finishInit];
		
		RHM_Comercial_TS_TEMATICA_META_DATA.table = @"\"rhm_comercial_1_0_ts_tematica\"";

 		[RHM_Comercial_TS_TEMATICA_META_DATA retain];
     	_valid_ = YES;

	}
	return RHM_Comercial_TS_TEMATICA_META_DATA;
}
...
- (void)merge:(RHM_Comercial_TS_TEMATICA*)entity withJson:(SUPJsonObject*)json
{
    ...
    if(![json containsKey:@"ZCODTEM"] )
    {
    	self.ZCODTEM = entity.ZCODTEM;
    }
    else if(![json getString:@"ZCODTEM"])
    {
    	self.ZCODTEM = entity.ZCODTEM;
    }
    if(![json containsKey:@"ZDESCR"] )
    ...
    if(![json containsKey:@"ZDESBR"] )
    {
    	self.ZDESBR = entity.ZDESBR;
    }
    else if(![json getString:@"ZDESBR"])
    {
    	self.ZDESBR = entity.ZDESBR;
    }
    if(![json containsKey:@"surrogateKey"] )
...
+ (RHM_Comercial_TS_TEMATICA*)getInstance
{
    RHM_Comercial_TS_TEMATICA* me = [[RHM_Comercial_TS_TEMATICA alloc] init];
    [me autorelease];
    return me;
}
...
- (SUPObjectList*)getLogRecords
{
    ...
    query.testCriteria = (SUPTestCriteria*)[[SUPAttributeTest match:@"component":@"TS_TEMATICA"] and:[SUPAttributeTest match:@"entityKey":[self keyToString]]];
...
- (void)refresh
{
    if (!self.isNew)
    {
        RHM_Comercial_TS_TEMATICA* entity = [RHM_Comercial_TS_TEMATICA load:[self id]];
        [self copyAll:entity];
	...
	for (id item in content)
    {
        ...
        if ([headers containsKey:@"log"]) 
        {
            for(SUPJsonObject *o in [headers getArray:@"log"])
			{
				...
				[RHM_Comercial_RHM_ComercialDB writeLogFromHeader:o:@"TS_TEMATICA":@"replay"];
			}
        }

        RHM_Comercial_TS_TEMATICA* clientEntity = [RHM_Comercial_TS_TEMATICA find:key];
        if( (clientEntity != nil) && (clientEntity.pending) )
        {
            ...
            if(client_rc == server_id)
            {
				...
				if([RHM_Comercial_TS_TEMATICA callbackHandler])
	        		[[RHM_Comercial_TS_TEMATICA callbackHandler] onReplaySuccess:clientEntity];       
...
+ (void)replayRejected:(SUPJsonObject*)headers withContent:(SUPJsonArray*)content
{
    for (id item in content)
    {
        ...
        if(clientEntity != nil)
        {
            ...
            if(client_rc == server_id)
            {
	        	RHM_Comercial_TS_TEMATICA* originalState = (RHM_Comercial_TS_TEMATICA*)clientEntity.originalState;
	        	SUPChar pendingChange = clientEntity.pendingChange;
				...
				clientEntity->_isDeleted = (pendingChange == PendingDelete);
			    if([RHM_Comercial_TS_TEMATICA callbackHandler])
			    	[[RHM_Comercial_TS_TEMATICA callbackHandler] onReplayFailure:clientEntity];
		        if([RHM_Comercial_RHM_ComercialDB callbackHandler])
...
-(void)submitPending1:(BOOL)doRefresh
{
    ...
    @try
    {
        ...
        if (self.pending && !self.disableSubmit && (self.replayCounter > self.replayPending))
        {
            ...
            [headers setString:@"mbo":@"TS_TEMATICA"];
    ...
    @finally
    {
        ...
        [[RHM_Comercial_RHM_ComercialDB dblock] unlock];
        if(success)
            [RHM_Comercial_RHM_ComercialDB replay:@"TS_TEMATICA" withId:self.replayCounter withContent:[message content]];
        [message release];
    }
}
...
- (void)createPending:(BOOL)isPending
{
    ...
    @try
    {
		...
		if (isPending)
		{
			RHM_Comercial_TS_TEMATICA* originalEntity = (RHM_Comercial_TS_TEMATICA*)[self originalState];
...
- (SUPAbstractEntity*)originalState
{
    ...
    {
        self->_originalState = self.pending ? [RHM_Comercial_TS_TEMATICA find:[self id] inTable:OriginalStateEntityTable] : nil;
...
- (SUPAbstractEntity*)downloadState
{
    if (!self->_downloadStateValid)
    {
        ...
        {
            RHM_Comercial_TS_TEMATICA *mbo = [RHM_Comercial_TS_TEMATICA find:[self id]];
            if(mbo)
...
- (void)updateOriginalState
{
    ...
    @try
    {
		...
		RHM_Comercial_TS_TEMATICA* os = [RHM_Comercial_TS_TEMATICA find:[self id] inTable:OriginalStateEntityTable];
		if(os)
		{
...
+ (SUPObjectList*)findAll:(int32_t)skip take:(int32_t)take
{
	...
	@try
	{
	    [[RHM_Comercial_RHM_ComercialDB dblock] readLock];
		connection = [RHM_Comercial_RHM_ComercialDB getConnectionWrapper];
		sql = [[NSMutableString alloc] initWithCapacity:283];
	    [sql appendString:@"SELECT x.\"a\",x.\"b\",x.\"c\",x.\"d\",x.\"_pf\",x.\"_pc\",x.\"_rp\",x.\"_rf\",x.\"e\",x.\"_rc\",x.\"_ds\" FROM \"rhm_comercial_1_0_ts_tematica\" x where (((x.\"_pf\" = 1 or not exists (select x_os.\"e\" from \"rhm_comercial_1_0_ts_tematica_os\" x_os where x_os.\"e\" = x.\"e\"))))"];
	    if(take != -1)
...
+ (SUPObjectList*)getPendingObjects:(int32_t)skip take:(int32_t)take
{
	...
	@try
	{
	    [[RHM_Comercial_RHM_ComercialDB dblock] readLock];
		connection = [RHM_Comercial_RHM_ComercialDB getConnectionWrapper];
		sql = [[NSMutableString alloc] initWithCapacity:201];
	    [sql appendString:@"select x.\"a\",x.\"b\",x.\"c\",x.\"d\",x.\"_pf\",x.\"_pc\",x.\"_rp\",x.\"_rf\",x.\"e\",x.\"_rc\",x.\"_ds\" from \"rhm_comercial_1_0_ts_tematica\" x where x.\"_pc\" <> \'N\' and x.\"_rc\" != x.\"_rp\""];
	    if(take != -1)
	        [sql appendFormat:@" limit %i",take];
	    if(skip != 0)
	        [sql appendFormat:@" offset %i",skip];
...
+ (SUPObjectList*)getPendingObjects:(unichar)pendingChange skip:(int32_t)skip take:(int32_t)take
{
	...
	@try
	{
	    [[RHM_Comercial_RHM_ComercialDB dblock] readLock];
		connection = [RHM_Comercial_RHM_ComercialDB getConnectionWrapper];
		sql = [[NSMutableString alloc] initWithCapacity:196];
	    [sql appendString:@"select x.\"a\",x.\"b\",x.\"c\",x.\"d\",x.\"_pf\",x.\"_pc\",x.\"_rp\",x.\"_rf\",x.\"e\",x.\"_rc\",x.\"_ds\" from \"rhm_comercial_1_0_ts_tematica\" x where x.\"_pc\" = ? and x.\"_rc\" != x.\"_rp\""];
	    if(take != -1)
	        [sql appendFormat:@" limit %i",take];
	    if(skip != 0)
	        [sql appendFormat:@" offset %i",skip];
...
+ (SUPObjectList*)getReplayPendingObjects:(int32_t)skip take:(int32_t)take
{
	...
	@try
	{
	    [[RHM_Comercial_RHM_ComercialDB dblock] readLock];
		connection = [RHM_Comercial_RHM_ComercialDB getConnectionWrapper];
		sql = [[NSMutableString alloc] initWithCapacity:305];
	    [sql appendString:@"select x.\"a\",x.\"b\",x.\"c\",x.\"d\",x.\"_pf\",x.\"_pc\",x.\"_rp\",x.\"_rf\",x.\"e\",x.\"_rc\",x.\"_ds\" from \"rhm_comercial_1_0_ts_tematica\" x where (((x.\"_pf\" = 1 or not exists (select x_os.\"e\" from \"rhm_comercial_1_0_ts_tematica_os\" x_os where x_os.\"e\" = x.\"e\")))) and ( x.\"_rp\" != 0)"];
	    if(take != -1)
	        [sql appendFormat:@" limit %i",take];
	    if(skip != 0)
	        [sql appendFormat:@" offset %i",skip];
...
{
    return [RHM_Comercial_TS_TEMATICA countWithQuery:query];
}
...
+ (int32_t)countReplayPendingObjects
{
	...
	@try
	{
	    ...
	    [sql appendString:@"select count(*) ,x.\"e\" from \"rhm_comercial_1_0_ts_tematica\" x where (((x.\"_pf\" = 1 or not exists (select x_os.\"e\" from \"rhm_comercial_1_0_ts_tematica_os\" x_os where x_os.\"e\" = x.\"e\")))) and ( x.\"_rp\" != 0)"];
