...
@interface RHM_Comercial_TS_ART_STOCK(private)
- (void)bind:(id<SUPResultSetWrapper>)resultSet;
- (void)bindWithQuery:(id<SUPResultSetWrapper>)resultSet:(SUPQuery*)query;
+ (RHM_Comercial_TS_ART_STOCK*)find:(int64_t)id inTable:(SUPEntityTable)table;
+ (void)importData:(RHM_Comercial_TS_ART_STOCK*)serverEntity:(SUPJsonObject*)jsonObject;
- (void)merge:(RHM_Comercial_TS_ART_STOCK*)entity withJson:(SUPJsonObject*)json;
- (void)setAllAttributesFrom:(RHM_Comercial_TS_ART_STOCK*)other;
@end
@implementation RHM_Comercial_TS_ART_STOCK

@synthesize BUKRS = _BUKRS;
@synthesize MATNR = _MATNR;
@synthesize WERKS = _WERKS;
@synthesize LGORT = _LGORT;
@synthesize FISICO = _FISICO;
@synthesize DISPONIBLE = _DISPONIBLE;
@synthesize PENDIENTE = _PENDIENTE;
@synthesize BLOQUEADO = _BLOQUEADO;
...
- (void)setWERKS:(NSString*)newWERKS
{
    if (newWERKS != self->_WERKS)
    {
		[self->_WERKS release];
        self->_WERKS = [newWERKS retain];
        self->_isDirty = YES;
    }
}
...
- (void)setFISICO:(NSNumber*)newFISICO
{
    if (newFISICO != self->_FISICO)
    {
		[self->_FISICO release];
        self->_FISICO = [newFISICO retain];
        self->_isDirty = YES;
    }
}

- (void)setDISPONIBLE:(NSNumber*)newDISPONIBLE
{
    if (newDISPONIBLE != self->_DISPONIBLE)
    {
		[self->_DISPONIBLE release];
        self->_DISPONIBLE = [newDISPONIBLE retain];
        self->_isDirty = YES;
    }
}

- (void)setPENDIENTE:(NSNumber*)newPENDIENTE
{
    if (newPENDIENTE != self->_PENDIENTE)
    {
		[self->_PENDIENTE release];
        self->_PENDIENTE = [newPENDIENTE retain];
        self->_isDirty = YES;
    }
}

- (void)setBLOQUEADO:(NSString*)newBLOQUEADO
{
    if (newBLOQUEADO != self->_BLOQUEADO)
    {
		[self->_BLOQUEADO release];
        self->_BLOQUEADO = [newBLOQUEADO retain];
        self->_isDirty = YES;
    }
}
...
- (void)readJson:(SUPJsonObject*)json:(int32_t)flags
{
        ...
        if ([json containsKey:@"MATNR"])
    {
        ...
        if ([json containsKey:@"WERKS"])
    {
        self.WERKS = [json getString:@"WERKS"];
    }
        if ([json containsKey:@"LGORT"])
    {
        self.LGORT = [json getString:@"LGORT"];
    }
        if ([json containsKey:@"FISICO"])
    {
        self.FISICO = [json getDecimal:@"FISICO"];
    }
        if ([json containsKey:@"DISPONIBLE"])
    {
        self.DISPONIBLE = [json getDecimal:@"DISPONIBLE"];
    }
        if ([json containsKey:@"PENDIENTE"])
    {
        self.PENDIENTE = [json getDecimal:@"PENDIENTE"];
    }
        if ([json containsKey:@"BLOQUEADO"])
    {
        self.BLOQUEADO = [json getString:@"BLOQUEADO"];
    }
...
- (SUPJsonObject*)json:(int32_t)flags
{
	...
	if (all)
	{
	    ...
	    [newJson setString:@"WERKS":self.WERKS];
	    [newJson setString:@"LGORT":self.LGORT];
	    [newJson setDecimal:@"FISICO":self.FISICO];
	    [newJson setDecimal:@"DISPONIBLE":self.DISPONIBLE];
	    [newJson setDecimal:@"PENDIENTE":self.PENDIENTE];
	    [newJson setString:@"BLOQUEADO":self.BLOQUEADO];
	    [newJson setBoolean:@"disableSubmit":self.disableSubmit];
	}
    [newJson setString:@"?":@"TS_ART_STOCK"];
    [newJson autorelease];
    return newJson;
}
+ (SUPObjectList*)fromJSONList:(SUPJsonArray*)jsonArray
{
	...
	{
	    RHM_Comercial_TS_ART_STOCK* inst = [[RHM_Comercial_TS_ART_STOCK alloc] initFromJson:jsonObject:0];
...
+ (SUPJsonArray*)toJSONList:(SUPObjectList*)instanceList
{
	...
	for (RHM_Comercial_TS_ART_STOCK* inst in instanceList)
	{
...
- (void)bind:(id<SUPResultSetWrapper>)resultSet
{
    @try
    {
        ...
        self.WERKS = [resultSet getString:3:@"c"];
        self.LGORT = [resultSet getString:4:@"d"];
        self.FISICO = [resultSet getDecimal:5:@"e"];
        self.DISPONIBLE = [resultSet getDecimal:6:@"f"];
        self.PENDIENTE = [resultSet getDecimal:7:@"g"];
        self.BLOQUEADO = [resultSet getString:8:@"h"];
...
- (void)bindWithQuery:(id<SUPResultSetWrapper>)resultSet:(SUPQuery*)query
{
    ...
    SUPEntityMetaData *entity = [RHM_Comercial_TS_ART_STOCK metaData];
    ...
    for(int j=0;j<[cl size];j++)
    {
		...
		if([amd.name isEqualToString:@"WERKS"])
        	self.WERKS = [resultSet getString:j+1:@"c"];
		if([amd.name isEqualToString:@"LGORT"])
        	self.LGORT = [resultSet getString:j+1:@"d"];
		if([amd.name isEqualToString:@"FISICO"])
        	self.FISICO = [resultSet getDecimal:j+1:@"e"];
		if([amd.name isEqualToString:@"DISPONIBLE"])
        	self.DISPONIBLE = [resultSet getDecimal:j+1:@"f"];
		if([amd.name isEqualToString:@"PENDIENTE"])
        	self.PENDIENTE = [resultSet getDecimal:j+1:@"g"];
		if([amd.name isEqualToString:@"BLOQUEADO"])
        	self.BLOQUEADO = [resultSet getString:j+1:@"h"];
...
- (void)copyAll:(RHM_Comercial_TS_ART_STOCK*)other
{
	[self copyAll:other withParent:(SUPAbstractEntity*)other];
}


- (void)copyAll:(RHM_Comercial_TS_ART_STOCK*)other withParent:(SUPAbstractEntity*)parent
{
    [self setBUKRS:other.BUKRS];
    
    [self setMATNR:other.MATNR];
    
    [self setWERKS:other.WERKS];
    
    [self setLGORT:other.LGORT];
    
    [self setFISICO:other.FISICO];
    
    [self setDISPONIBLE:other.DISPONIBLE];
    
    [self setPENDIENTE:other.PENDIENTE];
    
    [self setBLOQUEADO:other.BLOQUEADO];
...
id<SUPResultSetWrapper> resultSet = nil;
    {
        ...
        {    
            ...
            statement = [SUPStatementBuilder prepareQuery:connection:[RHM_Comercial_RHM_ComercialDB metaData]:[RHM_Comercial_TS_ART_STOCK metaData]:query];
...
+ (RHM_Comercial_TS_ART_STOCK*)find_by_id_string:(NSString*)id_string
{
    if(id_string == nil) return nil;
	int64_t key;
	NSNumberFormatter *nf = [[[NSNumberFormatter alloc] init] autorelease];
	key = [[nf numberFromString:id_string] longValue];
    return [RHM_Comercial_TS_ART_STOCK find:key];
	
}

+ (RHM_Comercial_TS_ART_STOCK*)find:(int64_t)id
{
	return [self find:id inTable:MainEntityTable];
}

+ (RHM_Comercial_TS_ART_STOCK*)find:(int64_t)id_ inTable:(SUPEntityTable)table
{
    RHM_Comercial_TS_ART_STOCK* result = nil;
	RHM_Comercial_TS_ART_STOCK* downloadState = nil;
    ...
    @try
    {
    	...
    	//@synchronized(connection)
    	{
        	@try
        	{
            	...
            	{
                	preparedStatement = [connection prepareStatement:@"select \"a\",\"b\",\"c\",\"d\",\"e\",\"f\",\"g\",\"h\",\"_pf\",\"_pc\",\"_rp\",\"_rf\",\"i\",\"_rc\",\"_ds\" from rhm_comercial_1_0_ts_art_stock where \"i\"=? order by \"_pf\""];
            	}
            	else
            	{
                	preparedStatement = [connection prepareStatement:@"select \"a\",\"b\",\"c\",\"d\",\"e\",\"f\",\"g\",\"h\",\"_pf\",\"_pc\",\"_rp\",\"_rf\",\"i\",\"_rc\",\"_ds\" from rhm_comercial_1_0_ts_art_stock_os where \"i\"=? order by \"_pf\""];
            	}
            	...
            	while ([resultSet next])
            	{
                	...
                	result = [[RHM_Comercial_TS_ART_STOCK alloc] init];
                	...
                	{
 	              		downloadState = [RHM_Comercial_TS_ART_STOCK getInstance];
...
id<SUPResultSetWrapper> resultSet = nil;
    {
        ...
        {    
            ...
            connection = [RHM_Comercial_RHM_ComercialDB getConnectionWrapper];
            int32_t count = 0;
            RHM_Comercial_TS_ART_STOCKList* list = [[RHM_Comercial_TS_ART_STOCKList alloc] init];
            ...
            {
                count++;   
                RHM_Comercial_TS_ART_STOCK* entity = [[RHM_Comercial_TS_ART_STOCK alloc] init];
...
- (id<SUPStatementWrapper>)getCreateStatement:(id<SUPConnectionWrapper>)connection forTable:(SUPEntityTable)table;
{
    ...
    if (table == MainEntityTable)
    {
        preparedStatement = [connection prepareStatement:@"insert into rhm_comercial_1_0_ts_art_stock (\"a\",\"b\",\"c\",\"d\",\"e\",\"f\",\"g\",\"h\",\"_pf\",\"_pc\",\"_rp\",\"_rf\",\"i\",\"_rc\",\"_ds\") values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"];
    }
    else
    {
        preparedStatement = [connection prepareStatement:@"insert into rhm_comercial_1_0_ts_art_stock_os (\"a\",\"b\",\"c\",\"d\",\"e\",\"f\",\"g\",\"h\",\"_pf\",\"_pc\",\"_rp\",\"_rf\",\"i\",\"_rc\",\"_ds\") values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"];
    }
    // Add updated values
	if(self.BUKRS == nil)
		self.BUKRS = @"";
		//RHM_Comercial_TS_ART_STOCK getCreateStatement:BUKRS not allowed to be null
    [preparedStatement setString:1:@"a":self.BUKRS];
	if(self.MATNR == nil)
		self.MATNR = @"";
		//RHM_Comercial_TS_ART_STOCK getCreateStatement:MATNR not allowed to be null
    [preparedStatement setString:2:@"b":self.MATNR];
	if(self.WERKS == nil)
		self.WERKS = @"";
		//RHM_Comercial_TS_ART_STOCK getCreateStatement:WERKS not allowed to be null
    [preparedStatement setString:3:@"c":self.WERKS];
	if(self.LGORT == nil)
		self.LGORT = @"";
		//RHM_Comercial_TS_ART_STOCK getCreateStatement:LGORT not allowed to be null
    [preparedStatement setString:4:@"d":self.LGORT];
	if(self.FISICO == nil)
		self.FISICO = [NSNumber numberWithFloat:0.0];
		//RHM_Comercial_TS_ART_STOCK getCreateStatement:FISICO not allowed to be null
    [preparedStatement setDecimal:5:@"e":self.FISICO];
	if(self.DISPONIBLE == nil)
		self.DISPONIBLE = [NSNumber numberWithFloat:0.0];
		//RHM_Comercial_TS_ART_STOCK getCreateStatement:DISPONIBLE not allowed to be null
    [preparedStatement setDecimal:6:@"f":self.DISPONIBLE];
	if(self.PENDIENTE == nil)
		self.PENDIENTE = [NSNumber numberWithFloat:0.0];
		//RHM_Comercial_TS_ART_STOCK getCreateStatement:PENDIENTE not allowed to be null
    [preparedStatement setDecimal:7:@"g":self.PENDIENTE];
	if(self.BLOQUEADO == nil)
		self.BLOQUEADO = @"";
		//RHM_Comercial_TS_ART_STOCK getCreateStatement:BLOQUEADO not allowed to be null
    [preparedStatement setString:8:@"h":self.BLOQUEADO];
...
- (id<SUPStatementWrapper>)getDeleteStatement:(id<SUPConnectionWrapper>)connection forTable:(SUPEntityTable)table;
{
    ...
    if (table == MainEntityTable)
    {
        preparedStatement = [connection prepareStatement:@"delete from rhm_comercial_1_0_ts_art_stock where \"_pf\"=? and \"i\"=?"];
    }
    else
    {
        preparedStatement = [connection prepareStatement:@"delete from rhm_comercial_1_0_ts_art_stock_os where \"_pf\"=? and \"i\"=?"];
    }
...
- (id<SUPStatementWrapper>)getUpdateStatement:(id<SUPConnectionWrapper>)connection forTable:(SUPEntityTable)table;
{
    ...
    if (table == MainEntityTable)
    {
    	preparedStatement = [connection prepareStatement:@"update rhm_comercial_1_0_ts_art_stock set \"a\"=?,\"b\"=?,\"c\"=?,\"d\"=?,\"e\"=?,\"f\"=?,\"g\"=?,\"h\"=?,\"_pc\"=?,\"_rp\"=?,\"_rf\"=?,\"_rc\"=?,\"_ds\"=? where \"_pf\"=? and \"i\"=?"];
    }
    else
    {
        preparedStatement = [connection prepareStatement:@"update rhm_comercial_1_0_ts_art_stock_os set \"a\"=?,\"b\"=?,\"c\"=?,\"d\"=?,\"e\"=?,\"f\"=?,\"g\"=?,\"h\"=?,\"_pc\"=?,\"_rp\"=?,\"_rf\"=?,\"_rc\"=?,\"_ds\"=? where \"_pf\"=? and \"i\"=?"];
    }
    // Add updated values
    int parameterIndex = 0;
	if(self.BUKRS == nil)
		self.BUKRS = @"";
		//RHM_Comercial_TS_ART_STOCK getUpdateStatement:BUKRS not allowed to be null
    parameterIndex++;
    [preparedStatement setString:parameterIndex:@"a":self.BUKRS];
	if(self.MATNR == nil)
		self.MATNR = @"";
		//RHM_Comercial_TS_ART_STOCK getUpdateStatement:MATNR not allowed to be null
    parameterIndex++;
    [preparedStatement setString:parameterIndex:@"b":self.MATNR];
	if(self.WERKS == nil)
		self.WERKS = @"";
		//RHM_Comercial_TS_ART_STOCK getUpdateStatement:WERKS not allowed to be null
    parameterIndex++;
    [preparedStatement setString:parameterIndex:@"c":self.WERKS];
	if(self.LGORT == nil)
		self.LGORT = @"";
		//RHM_Comercial_TS_ART_STOCK getUpdateStatement:LGORT not allowed to be null
    parameterIndex++;
    [preparedStatement setString:parameterIndex:@"d":self.LGORT];
	if(self.FISICO == nil)
		self.FISICO = [NSNumber numberWithFloat:0.0];
		//RHM_Comercial_TS_ART_STOCK getUpdateStatement:FISICO not allowed to be null
    parameterIndex++;
    [preparedStatement setDecimal:parameterIndex:@"e":self.FISICO];
	if(self.DISPONIBLE == nil)
		self.DISPONIBLE = [NSNumber numberWithFloat:0.0];
		//RHM_Comercial_TS_ART_STOCK getUpdateStatement:DISPONIBLE not allowed to be null
    parameterIndex++;
    [preparedStatement setDecimal:parameterIndex:@"f":self.DISPONIBLE];
	if(self.PENDIENTE == nil)
		self.PENDIENTE = [NSNumber numberWithFloat:0.0];
		//RHM_Comercial_TS_ART_STOCK getUpdateStatement:PENDIENTE not allowed to be null
    parameterIndex++;
    [preparedStatement setDecimal:parameterIndex:@"g":self.PENDIENTE];
	if(self.BLOQUEADO == nil)
		self.BLOQUEADO = @"";
		//RHM_Comercial_TS_ART_STOCK getUpdateStatement:BLOQUEADO not allowed to be null
    parameterIndex++;
    [preparedStatement setString:parameterIndex:@"h":self.BLOQUEADO];
    parameterIndex++;
...
+ (void)importData:(RHM_Comercial_TS_ART_STOCK*)serverEntity:(SUPJsonObject*)jsonObject
{
	...
	RHM_Comercial_TS_ART_STOCK* serverEntity2 = [RHM_Comercial_TS_ART_STOCK getInstance];
    ...
    if([RHM_Comercial_TS_ART_STOCK callbackHandler])
		[[RHM_Comercial_TS_ART_STOCK callbackHandler] beforeImport:serverEntity2];
    ...
    } else if(delete)
    {
        ...
        if(isCreated) 
        {
            RHM_Comercial_TS_ART_STOCK *clientEntity = [RHM_Comercial_TS_ART_STOCK find:[serverEntity _pk]];
	...
	if(update)
    {
    	...
    	if(clientEntity.pending)
    		clientEntity = (RHM_Comercial_TS_ART_STOCK*)clientEntity.downloadState;
        @try 
        {
    ...
    if([RHM_Comercial_TS_ART_STOCK callbackHandler])
		[[RHM_Comercial_TS_ART_STOCK callbackHandler] onImport:serverEntity];
...
+ (RHM_Comercial_TS_ART_STOCK*)load:(int64_t)id
{
	RHM_Comercial_TS_ART_STOCK* entity = [self find:id inTable:MainEntityTable];
	//RHM_Comercial_TS_ART_STOCK* entity = [self find:id];
    if (entity == nil)
    {
		@throw [SUPObjectNotFoundException withMessage:@"-load: in RHM_Comercial_TS_ART_STOCK: entity not found in database"];
    }
    return entity;
}

static SUPEntityMetaData* RHM_Comercial_TS_ART_STOCK_META_DATA;
...
+ (SUPEntityMetaData*)metaData
{
    if (! (_valid_))
    {
       RHM_Comercial_TS_ART_STOCK_META_DATA = [SUPEntityMetaData getInstance];
       RHM_Comercial_TS_ART_STOCK_META_DATA.id = _ENTITY_NUMBER_FOR_RHM_Comercial_TS_ART_STOCK_;
       RHM_Comercial_TS_ART_STOCK_META_DATA.name = @"TS_ART_STOCK";
       [RHM_Comercial_TS_ART_STOCK_META_DATA setClass:[RHM_Comercial_TS_ART_STOCK class]];

	   RHM_Comercial_TS_ART_STOCK_META_DATA.isClientOnly = NO;
       ...
       // Handle attributes
		...
		[a_MATNR setScale:0];
		[a_MATNR setIsKey:NO];
		SUPAttributeMetaData_DC* a_WERKS = [RHM_Comercial_RHM_ComercialDB createAttributeMetaData:3:(SUPString)@"WERKS":[SUPDataType forName:@"string"]];
		[a_WERKS setColumn:@"\"c\""];
		[a_WERKS setMaxLength:4];
		[a_WERKS setPrecision:0];
		[a_WERKS setScale:0];
		[a_WERKS setIsKey:NO];
		SUPAttributeMetaData_DC* a_LGORT = [RHM_Comercial_RHM_ComercialDB createAttributeMetaData:4:(SUPString)@"LGORT":[SUPDataType forName:@"string"]];
		[a_LGORT setColumn:@"\"d\""];
		[a_LGORT setMaxLength:4];
		[a_LGORT setPrecision:0];
		[a_LGORT setScale:0];
		[a_LGORT setIsKey:NO];
		SUPAttributeMetaData_DC* a_FISICO = [RHM_Comercial_RHM_ComercialDB createAttributeMetaData:5:(SUPString)@"FISICO":[SUPDataType forName:@"decimal"]];
		[a_FISICO setColumn:@"\"e\""];
		[a_FISICO setMaxLength:-1];
		[a_FISICO setPrecision:28];
		[a_FISICO setScale:8];
		[a_FISICO setIsKey:NO];
		SUPAttributeMetaData_DC* a_DISPONIBLE = [RHM_Comercial_RHM_ComercialDB createAttributeMetaData:6:(SUPString)@"DISPONIBLE":[SUPDataType forName:@"decimal"]];
		[a_DISPONIBLE setColumn:@"\"f\""];
		[a_DISPONIBLE setMaxLength:-1];
		[a_DISPONIBLE setPrecision:28];
		[a_DISPONIBLE setScale:8];
		[a_DISPONIBLE setIsKey:NO];
		SUPAttributeMetaData_DC* a_PENDIENTE = [RHM_Comercial_RHM_ComercialDB createAttributeMetaData:7:(SUPString)@"PENDIENTE":[SUPDataType forName:@"decimal"]];
		[a_PENDIENTE setColumn:@"\"g\""];
		[a_PENDIENTE setMaxLength:-1];
		[a_PENDIENTE setPrecision:28];
		[a_PENDIENTE setScale:8];
		[a_PENDIENTE setIsKey:NO];
		SUPAttributeMetaData_DC* a_BLOQUEADO = [RHM_Comercial_RHM_ComercialDB createAttributeMetaData:8:(SUPString)@"BLOQUEADO":[SUPDataType forName:@"string"]];
		[a_BLOQUEADO setColumn:@"\"h\""];
		[a_BLOQUEADO setMaxLength:1];
		[a_BLOQUEADO setPrecision:0];
		[a_BLOQUEADO setScale:0];
		[a_BLOQUEADO setIsKey:NO];
		...
		[a_disableSubmit setIsKey:NO];
 		...
 		[attributes addThis:a_WERKS];
 		[attributes addThis:a_LGORT];
 		[attributes addThis:a_FISICO];
 		[attributes addThis:a_DISPONIBLE];
 		[attributes addThis:a_PENDIENTE];
 		[attributes addThis:a_BLOQUEADO];
 		...
 		RHM_Comercial_TS_ART_STOCK_META_DATA.attributes = attributes;
 		
 		SUPAttributeMap * attributeMap = [SUPAttributeMap getInstance];
    	[attributeMap setAttributes:attributes];
    	RHM_Comercial_TS_ART_STOCK_META_DATA.attributeMap = [attributeMap finishInit];
 
 		// Handle operations
 		SUPOperationMetaData* o_findAll_0 = [RHM_Comercial_RHM_ComercialDB createOperationMetaData:1:(SUPString)@"findAll":[SUPDataType forName:@"TS_ART_STOCK*"]:true];
		...
		[o_findAll_0 setIsDelete:NO]; 		
 		...
 		SUPOperationMetaData* o_getPendingObjects_0 = [RHM_Comercial_RHM_ComercialDB createOperationMetaData:2:(SUPString)@"getPendingObjects":[SUPDataType forName:@"TS_ART_STOCK*"]:true];
		...
		[o_getPendingObjects_0 setIsDelete:NO]; 		
 		...
 		SUPOperationMetaData* o_getPendingObjects_1 = [RHM_Comercial_RHM_ComercialDB createOperationMetaData:3:(SUPString)@"getPendingObjects":[SUPDataType forName:@"TS_ART_STOCK*"]:true];
  		{
		...
		[o_getPendingObjects_1 setIsDelete:NO]; 		
 		...
 		SUPOperationMetaData* o_getReplayPendingObjects_0 = [RHM_Comercial_RHM_ComercialDB createOperationMetaData:4:(SUPString)@"getReplayPendingObjects":[SUPDataType forName:@"TS_ART_STOCK*"]:true];
		...
		[o_save_0 setIsDelete:NO]; 		
 		[o_save_0 finishInit];
 		SUPOperationMetaData* o_findWithQuery_1 = [RHM_Comercial_RHM_ComercialDB createOperationMetaData:12:(SUPString)@"findWithQuery":[SUPDataType forName:@"TS_ART_STOCK*"]:true];
  		{
 			SUPObjectList *parameters_list = nil;
		...
		[o_countReplayPendingObjects_0 setIsDelete:NO]; 		
 		...
 		RHM_Comercial_TS_ART_STOCK_META_DATA.operations = operations;
 		
		SUPOperationMap *operationMap = [SUPOperationMap getInstance];
		[operationMap setOperations:operations];
		RHM_Comercial_TS_ART_STOCK_META_DATA.operationMap = [operationMap finishInit];
		
		RHM_Comercial_TS_ART_STOCK_META_DATA.table = @"\"rhm_comercial_1_0_ts_art_stock\"";

 		[RHM_Comercial_TS_ART_STOCK_META_DATA retain];
     	_valid_ = YES;

	}
	return RHM_Comercial_TS_ART_STOCK_META_DATA;
}
...
- (void)merge:(RHM_Comercial_TS_ART_STOCK*)entity withJson:(SUPJsonObject*)json
{
    ...
    if(![json containsKey:@"WERKS"] )
    {
    	self.WERKS = entity.WERKS;
    }
    else if(![json getString:@"WERKS"])
    {
    	self.WERKS = entity.WERKS;
    }
    if(![json containsKey:@"LGORT"] )
    {
    	self.LGORT = entity.LGORT;
    }
    else if(![json getString:@"LGORT"])
    {
    	self.LGORT = entity.LGORT;
    }
    if(![json containsKey:@"FISICO"] )
    {
    	self.FISICO = entity.FISICO;
    }
    else if(![json getDecimal:@"FISICO"])
    {
    	self.FISICO = entity.FISICO;
    }
    if(![json containsKey:@"DISPONIBLE"] )
    {
    	self.DISPONIBLE = entity.DISPONIBLE;
    }
    else if(![json getDecimal:@"DISPONIBLE"])
    {
    	self.DISPONIBLE = entity.DISPONIBLE;
    }
    if(![json containsKey:@"PENDIENTE"] )
    {
    	self.PENDIENTE = entity.PENDIENTE;
    }
    else if(![json getDecimal:@"PENDIENTE"])
    {
    	self.PENDIENTE = entity.PENDIENTE;
    }
    if(![json containsKey:@"BLOQUEADO"] )
    {
    	self.BLOQUEADO = entity.BLOQUEADO;
    }
    else if(![json getString:@"BLOQUEADO"])
    {
    	self.BLOQUEADO = entity.BLOQUEADO;
    }
    if(![json containsKey:@"surrogateKey"] )
...
+ (RHM_Comercial_TS_ART_STOCK*)getInstance
{
    RHM_Comercial_TS_ART_STOCK* me = [[RHM_Comercial_TS_ART_STOCK alloc] init];
    [me autorelease];
    return me;
}
...
- (SUPObjectList*)getLogRecords
{
    ...
    query.testCriteria = (SUPTestCriteria*)[[SUPAttributeTest match:@"component":@"TS_ART_STOCK"] and:[SUPAttributeTest match:@"entityKey":[self keyToString]]];
...
- (void)refresh
{
    if (!self.isNew)
    {
        RHM_Comercial_TS_ART_STOCK* entity = [RHM_Comercial_TS_ART_STOCK load:[self id]];
        [self copyAll:entity];
	...
	for (id item in content)
    {
        ...
        if ([headers containsKey:@"log"]) 
        {
            for(SUPJsonObject *o in [headers getArray:@"log"])
			{
				...
				[RHM_Comercial_RHM_ComercialDB writeLogFromHeader:o:@"TS_ART_STOCK":@"replay"];
			}
        }

        RHM_Comercial_TS_ART_STOCK* clientEntity = [RHM_Comercial_TS_ART_STOCK find:key];
        if( (clientEntity != nil) && (clientEntity.pending) )
        {
            ...
            if(client_rc == server_id)
            {
				...
				if([RHM_Comercial_TS_ART_STOCK callbackHandler])
	        		[[RHM_Comercial_TS_ART_STOCK callbackHandler] onReplaySuccess:clientEntity];       
...
+ (void)replayRejected:(SUPJsonObject*)headers withContent:(SUPJsonArray*)content
{
    for (id item in content)
    {
        ...
        if(clientEntity != nil)
        {
            ...
            if(client_rc == server_id)
            {
	        	RHM_Comercial_TS_ART_STOCK* originalState = (RHM_Comercial_TS_ART_STOCK*)clientEntity.originalState;
	        	SUPChar pendingChange = clientEntity.pendingChange;
				...
				clientEntity->_isDeleted = (pendingChange == PendingDelete);
			    if([RHM_Comercial_TS_ART_STOCK callbackHandler])
			    	[[RHM_Comercial_TS_ART_STOCK callbackHandler] onReplayFailure:clientEntity];
		        if([RHM_Comercial_RHM_ComercialDB callbackHandler])
...
-(void)submitPending1:(BOOL)doRefresh
{
    ...
    @try
    {
        ...
        if (self.pending && !self.disableSubmit && (self.replayCounter > self.replayPending))
        {
            ...
            [headers setString:@"mbo":@"TS_ART_STOCK"];
    ...
    @finally
    {
        ...
        [[RHM_Comercial_RHM_ComercialDB dblock] unlock];
        if(success)
            [RHM_Comercial_RHM_ComercialDB replay:@"TS_ART_STOCK" withId:self.replayCounter withContent:[message content]];
        [message release];
    }
}
...
- (void)createPending:(BOOL)isPending
{
    ...
    @try
    {
		...
		if (isPending)
		{
			RHM_Comercial_TS_ART_STOCK* originalEntity = (RHM_Comercial_TS_ART_STOCK*)[self originalState];
...
- (SUPAbstractEntity*)originalState
{
    ...
    {
        self->_originalState = self.pending ? [RHM_Comercial_TS_ART_STOCK find:[self id] inTable:OriginalStateEntityTable] : nil;
...
- (SUPAbstractEntity*)downloadState
{
    if (!self->_downloadStateValid)
    {
        ...
        {
            RHM_Comercial_TS_ART_STOCK *mbo = [RHM_Comercial_TS_ART_STOCK find:[self id]];
            if(mbo)
...
- (void)updateOriginalState
{
    ...
    @try
    {
		...
		RHM_Comercial_TS_ART_STOCK* os = [RHM_Comercial_TS_ART_STOCK find:[self id] inTable:OriginalStateEntityTable];
		if(os)
		{
...
+ (SUPObjectList*)findAll:(int32_t)skip take:(int32_t)take
{
	...
	@try
	{
	    [[RHM_Comercial_RHM_ComercialDB dblock] readLock];
		connection = [RHM_Comercial_RHM_ComercialDB getConnectionWrapper];
		sql = [[NSMutableString alloc] initWithCapacity:326];
	    [sql appendString:@"SELECT x.\"a\",x.\"b\",x.\"c\",x.\"d\",x.\"e\",x.\"f\",x.\"g\",x.\"h\",x.\"_pf\",x.\"_pc\",x.\"_rp\",x.\"_rf\",x.\"i\",x.\"_rc\",x.\"_ds\" FROM \"rhm_comercial_1_0_ts_art_stock\" x where (((x.\"_pf\" = 1 or not exists (select x_os.\"i\" from \"rhm_comercial_1_0_ts_art_stock_os\" x_os where x_os.\"i\" = x.\"i\")"
	                            ")))"];
	    if(take != -1)
...
+ (SUPObjectList*)getPendingObjects:(int32_t)skip take:(int32_t)take
{
	...
	@try
	{
	    [[RHM_Comercial_RHM_ComercialDB dblock] readLock];
		connection = [RHM_Comercial_RHM_ComercialDB getConnectionWrapper];
		sql = [[NSMutableString alloc] initWithCapacity:234];
	    [sql appendString:@"select x.\"a\",x.\"b\",x.\"c\",x.\"d\",x.\"e\",x.\"f\",x.\"g\",x.\"h\",x.\"_pf\",x.\"_pc\",x.\"_rp\",x.\"_rf\",x.\"i\",x.\"_rc\",x.\"_ds\" from \"rhm_comercial_1_0_ts_art_stock\" x where x.\"_pc\" <> \'N\' and x.\"_rc\" != x.\"_rp\""];
	    if(take != -1)
	        [sql appendFormat:@" limit %i",take];
	    if(skip != 0)
	        [sql appendFormat:@" offset %i",skip];
...
+ (SUPObjectList*)getPendingObjects:(unichar)pendingChange skip:(int32_t)skip take:(int32_t)take
{
	...
	@try
	{
	    [[RHM_Comercial_RHM_ComercialDB dblock] readLock];
		connection = [RHM_Comercial_RHM_ComercialDB getConnectionWrapper];
		sql = [[NSMutableString alloc] initWithCapacity:229];
	    [sql appendString:@"select x.\"a\",x.\"b\",x.\"c\",x.\"d\",x.\"e\",x.\"f\",x.\"g\",x.\"h\",x.\"_pf\",x.\"_pc\",x.\"_rp\",x.\"_rf\",x.\"i\",x.\"_rc\",x.\"_ds\" from \"rhm_comercial_1_0_ts_art_stock\" x where x.\"_pc\" = ? and x.\"_rc\" != x.\"_rp\""];
	    if(take != -1)
	        [sql appendFormat:@" limit %i",take];
	    if(skip != 0)
	        [sql appendFormat:@" offset %i",skip];
...
+ (SUPObjectList*)getReplayPendingObjects:(int32_t)skip take:(int32_t)take
{
	...
	@try
	{
	    [[RHM_Comercial_RHM_ComercialDB dblock] readLock];
		connection = [RHM_Comercial_RHM_ComercialDB getConnectionWrapper];
		sql = [[NSMutableString alloc] initWithCapacity:348];
	    [sql appendString:@"select x.\"a\",x.\"b\",x.\"c\",x.\"d\",x.\"e\",x.\"f\",x.\"g\",x.\"h\",x.\"_pf\",x.\"_pc\",x.\"_rp\",x.\"_rf\",x.\"i\",x.\"_rc\",x.\"_ds\" from \"rhm_comercial_1_0_ts_art_stock\" x where (((x.\"_pf\" = 1 or not exists (select x_os.\"i\" from \"rhm_comercial_1_0_ts_art_stock_os\" x_os where x_os.\"i\" = x.\"i\")"
	                            "))) and ( x.\"_rp\" != 0)"];
	    if(take != -1)
	        [sql appendFormat:@" limit %i",take];
...
{
    return [RHM_Comercial_TS_ART_STOCK countWithQuery:query];
}
...
+ (int32_t)countReplayPendingObjects
{
	...
	@try
	{
	    ...
	    [sql appendString:@"select count(*) ,x.\"i\" from \"rhm_comercial_1_0_ts_art_stock\" x where (((x.\"_pf\" = 1 or not exists (select x_os.\"i\" from \"rhm_comercial_1_0_ts_art_stock_os\" x_os where x_os.\"i\" = x.\"i\")))) and ( x.\"_rp\" != 0)"];
