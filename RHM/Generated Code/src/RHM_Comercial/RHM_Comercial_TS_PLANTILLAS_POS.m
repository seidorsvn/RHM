...
@interface RHM_Comercial_TS_PLANTILLAS_POS(private)
- (void)bind:(id<SUPResultSetWrapper>)resultSet;
- (void)bindWithQuery:(id<SUPResultSetWrapper>)resultSet:(SUPQuery*)query;
+ (RHM_Comercial_TS_PLANTILLAS_POS*)find:(int64_t)id inTable:(SUPEntityTable)table;
+ (void)importData:(RHM_Comercial_TS_PLANTILLAS_POS*)serverEntity:(SUPJsonObject*)jsonObject;
- (void)merge:(RHM_Comercial_TS_PLANTILLAS_POS*)entity withJson:(SUPJsonObject*)json;
- (void)setAllAttributesFrom:(RHM_Comercial_TS_PLANTILLAS_POS*)other;
@end
@implementation RHM_Comercial_TS_PLANTILLAS_POS

@synthesize BUKRS = _BUKRS;
@synthesize PLANTILLA = _PLANTILLA;
@synthesize POSICION = _POSICION;
@synthesize MATNR = _MATNR;
@synthesize CANTIDAD = _CANTIDAD;
@synthesize UNIDAD = _UNIDAD;
...
- (void)setPLANTILLA:(NSString*)newPLANTILLA
{
    if (newPLANTILLA != self->_PLANTILLA)
    {
		[self->_PLANTILLA release];
        self->_PLANTILLA = [newPLANTILLA retain];
        self->_isDirty = YES;
    }
}

- (void)setPOSICION:(NSString*)newPOSICION
{
    if (newPOSICION != self->_POSICION)
    {
		[self->_POSICION release];
        self->_POSICION = [newPOSICION retain];
        self->_isDirty = YES;
    }
}
...
- (void)setCANTIDAD:(NSNumber*)newCANTIDAD
{
    if (newCANTIDAD != self->_CANTIDAD)
    {
		[self->_CANTIDAD release];
        self->_CANTIDAD = [newCANTIDAD retain];
        self->_isDirty = YES;
    }
}

- (void)setUNIDAD:(NSString*)newUNIDAD
{
    if (newUNIDAD != self->_UNIDAD)
    {
		[self->_UNIDAD release];
        self->_UNIDAD = [newUNIDAD retain];
        self->_isDirty = YES;
    }
}
...
- (void)readJson:(SUPJsonObject*)json:(int32_t)flags
{
        if ([json containsKey:@"BUKRS"])
    {
        ...
        if ([json containsKey:@"PLANTILLA"])
    {
        self.PLANTILLA = [json getString:@"PLANTILLA"];
    }
        if ([json containsKey:@"POSICION"])
    {
        self.POSICION = [json getString:@"POSICION"];
    }
    ...
    {
        self.CANTIDAD = [json getDecimal:@"CANTIDAD"];
    }
        if ([json containsKey:@"UNIDAD"])
    {
        self.UNIDAD = [json getString:@"UNIDAD"];
    }
...
- (SUPJsonObject*)json:(int32_t)flags
{
	...
	if (all)
	{
	    ...
	    [newJson setString:@"PLANTILLA":self.PLANTILLA];
	    [newJson setString:@"POSICION":self.POSICION];
	    [newJson setString:@"MATNR":self.MATNR];
	    [newJson setDecimal:@"CANTIDAD":self.CANTIDAD];
	    [newJson setString:@"UNIDAD":self.UNIDAD];
	    [newJson setBoolean:@"disableSubmit":self.disableSubmit];
	}
    [newJson setString:@"?":@"TS_PLANTILLAS_POS"];
    [newJson autorelease];
    return newJson;
}
+ (SUPObjectList*)fromJSONList:(SUPJsonArray*)jsonArray
{
	...
	{
	    RHM_Comercial_TS_PLANTILLAS_POS* inst = [[RHM_Comercial_TS_PLANTILLAS_POS alloc] initFromJson:jsonObject:0];
...
+ (SUPJsonArray*)toJSONList:(SUPObjectList*)instanceList
{
	...
	for (RHM_Comercial_TS_PLANTILLAS_POS* inst in instanceList)
	{
...
- (void)bind:(id<SUPResultSetWrapper>)resultSet
{
    @try
    {
        ...
        self.PLANTILLA = [resultSet getString:2:@"b"];
        self.POSICION = [resultSet getString:3:@"c"];
        self.MATNR = [resultSet getString:4:@"d"];
        self.CANTIDAD = [resultSet getDecimal:5:@"e"];
        self.UNIDAD = [resultSet getString:6:@"f"];
        self->_pending = [resultSet getBoolean:7:@"_pf"];
        self->_pendingChange = [resultSet getChar:8:@"_pc"];
        self->_replayPending = [resultSet getLong:9:@"_rp"];
        self->_replayFailure = [resultSet getLong:10:@"_rf"];
        self.surrogateKey = [resultSet getLong:11:@"g"];
        self->_replayCounter = [resultSet getLong:12:@"_rc"];
        self.disableSubmit = [resultSet getBoolean:13:@"_ds"];
...
- (void)bindWithQuery:(id<SUPResultSetWrapper>)resultSet:(SUPQuery*)query
{
    ...
    SUPEntityMetaData *entity = [RHM_Comercial_TS_PLANTILLAS_POS metaData];
    ...
    for(int j=0;j<[cl size];j++)
    {
		...
		if([amd.name isEqualToString:@"PLANTILLA"])
        	self.PLANTILLA = [resultSet getString:j+1:@"b"];
		if([amd.name isEqualToString:@"POSICION"])
        	self.POSICION = [resultSet getString:j+1:@"c"];
		if([amd.name isEqualToString:@"MATNR"])
        	self.MATNR = [resultSet getString:j+1:@"d"];
		if([amd.name isEqualToString:@"CANTIDAD"])
        	self.CANTIDAD = [resultSet getDecimal:j+1:@"e"];
		if([amd.name isEqualToString:@"UNIDAD"])
        	self.UNIDAD = [resultSet getString:j+1:@"f"];
		...
		if([amd.name isEqualToString:@"surrogateKey"])
        	self.surrogateKey = [resultSet getLong:j+1:@"g"];
...
- (void)copyAll:(RHM_Comercial_TS_PLANTILLAS_POS*)other
{
	[self copyAll:other withParent:(SUPAbstractEntity*)other];
}


- (void)copyAll:(RHM_Comercial_TS_PLANTILLAS_POS*)other withParent:(SUPAbstractEntity*)parent
{
    [self setBUKRS:other.BUKRS];
    
    [self setPLANTILLA:other.PLANTILLA];
    
    [self setPOSICION:other.POSICION];
    
    [self setMATNR:other.MATNR];
    
    [self setCANTIDAD:other.CANTIDAD];
    
    [self setUNIDAD:other.UNIDAD];
...
id<SUPResultSetWrapper> resultSet = nil;
    {
        ...
        {    
            ...
            statement = [SUPStatementBuilder prepareQuery:connection:[RHM_Comercial_RHM_ComercialDB metaData]:[RHM_Comercial_TS_PLANTILLAS_POS metaData]:query];
...
+ (RHM_Comercial_TS_PLANTILLAS_POS*)find_by_id_string:(NSString*)id_string
{
    if(id_string == nil) return nil;
	int64_t key;
	NSNumberFormatter *nf = [[[NSNumberFormatter alloc] init] autorelease];
	key = [[nf numberFromString:id_string] longValue];
    return [RHM_Comercial_TS_PLANTILLAS_POS find:key];
	
}

+ (RHM_Comercial_TS_PLANTILLAS_POS*)find:(int64_t)id
{
	return [self find:id inTable:MainEntityTable];
}

+ (RHM_Comercial_TS_PLANTILLAS_POS*)find:(int64_t)id_ inTable:(SUPEntityTable)table
{
    RHM_Comercial_TS_PLANTILLAS_POS* result = nil;
	RHM_Comercial_TS_PLANTILLAS_POS* downloadState = nil;
    ...
    @try
    {
    	...
    	//@synchronized(connection)
    	{
        	@try
        	{
            	...
            	{
                	preparedStatement = [connection prepareStatement:@"select \"a\",\"b\",\"c\",\"d\",\"e\",\"f\",\"_pf\",\"_pc\",\"_rp\",\"_rf\",\"g\",\"_rc\",\"_ds\" from rhm_comercial_1_0_ts_plantillas_pos where \"g\"=? order by \"_pf\""];
            	}
            	else
            	{
                	preparedStatement = [connection prepareStatement:@"select \"a\",\"b\",\"c\",\"d\",\"e\",\"f\",\"_pf\",\"_pc\",\"_rp\",\"_rf\",\"g\",\"_rc\",\"_ds\" from rhm_comercial_1_0_ts_plantillas_pos_os where \"g\"=? order by \"_pf\""];
            	}
            	...
            	while ([resultSet next])
            	{
                	...
                	result = [[RHM_Comercial_TS_PLANTILLAS_POS alloc] init];
                	...
                	{
 	              		downloadState = [RHM_Comercial_TS_PLANTILLAS_POS getInstance];
...
id<SUPResultSetWrapper> resultSet = nil;
    {
        ...
        {    
            ...
            connection = [RHM_Comercial_RHM_ComercialDB getConnectionWrapper];
            int32_t count = 0;
            RHM_Comercial_TS_PLANTILLAS_POSList* list = [[RHM_Comercial_TS_PLANTILLAS_POSList alloc] init];
            ...
            {
                count++;   
                RHM_Comercial_TS_PLANTILLAS_POS* entity = [[RHM_Comercial_TS_PLANTILLAS_POS alloc] init];
...
- (id<SUPStatementWrapper>)getCreateStatement:(id<SUPConnectionWrapper>)connection forTable:(SUPEntityTable)table;
{
    ...
    if (table == MainEntityTable)
    {
        preparedStatement = [connection prepareStatement:@"insert into rhm_comercial_1_0_ts_plantillas_pos (\"a\",\"b\",\"c\",\"d\",\"e\",\"f\",\"_pf\",\"_pc\",\"_rp\",\"_rf\",\"g\",\"_rc\",\"_ds\") values (?,?,?,?,?,?,?,?,?,?,?,?,?)"];
    }
    else
    {
        preparedStatement = [connection prepareStatement:@"insert into rhm_comercial_1_0_ts_plantillas_pos_os (\"a\",\"b\",\"c\",\"d\",\"e\",\"f\",\"_pf\",\"_pc\",\"_rp\",\"_rf\",\"g\",\"_rc\",\"_ds\") values (?,?,?,?,?,?,?,?,?,?,?,?,?)"];
    }
    // Add updated values
	if(self.BUKRS == nil)
		self.BUKRS = @"";
		//RHM_Comercial_TS_PLANTILLAS_POS getCreateStatement:BUKRS not allowed to be null
    [preparedStatement setString:1:@"a":self.BUKRS];
	if(self.PLANTILLA == nil)
		self.PLANTILLA = @"";
		//RHM_Comercial_TS_PLANTILLAS_POS getCreateStatement:PLANTILLA not allowed to be null
    [preparedStatement setString:2:@"b":self.PLANTILLA];
	if(self.POSICION == nil)
		self.POSICION = @"";
		//RHM_Comercial_TS_PLANTILLAS_POS getCreateStatement:POSICION not allowed to be null
    [preparedStatement setString:3:@"c":self.POSICION];
	if(self.MATNR == nil)
		self.MATNR = @"";
		//RHM_Comercial_TS_PLANTILLAS_POS getCreateStatement:MATNR not allowed to be null
    [preparedStatement setString:4:@"d":self.MATNR];
	if(self.CANTIDAD == nil)
		self.CANTIDAD = [NSNumber numberWithFloat:0.0];
		//RHM_Comercial_TS_PLANTILLAS_POS getCreateStatement:CANTIDAD not allowed to be null
    [preparedStatement setDecimal:5:@"e":self.CANTIDAD];
	if(self.UNIDAD == nil)
		self.UNIDAD = @"";
		//RHM_Comercial_TS_PLANTILLAS_POS getCreateStatement:UNIDAD not allowed to be null
    [preparedStatement setString:6:@"f":self.UNIDAD];
    [preparedStatement setBoolean:7:@"_pf":((table == MainEntityTable) ? self->_pending : NO)];    		
    [preparedStatement setChar:8:@"_pc":((table == MainEntityTable) ? self->_pendingChange : 'N')];    		
    [preparedStatement setLong:9:@"_rp":((table == MainEntityTable) ? self->_replayPending : 0)];    		
    [preparedStatement setLong:10:@"_rf":((table == MainEntityTable) ? self->_replayFailure : 0)];    		
    [preparedStatement setLong:11:@"g":self.surrogateKey];
    [preparedStatement setLong:12:@"_rc":((table == MainEntityTable) ? self.replayCounter : 0)];
    [preparedStatement setBoolean:13:@"_ds":self.disableSubmit];
...
- (id<SUPStatementWrapper>)getDeleteStatement:(id<SUPConnectionWrapper>)connection forTable:(SUPEntityTable)table;
{
    ...
    if (table == MainEntityTable)
    {
        preparedStatement = [connection prepareStatement:@"delete from rhm_comercial_1_0_ts_plantillas_pos where \"_pf\"=? and \"g\"=?"];
    }
    else
    {
        preparedStatement = [connection prepareStatement:@"delete from rhm_comercial_1_0_ts_plantillas_pos_os where \"_pf\"=? and \"g\"=?"];
    }
    // Fill in values for "where" clause
    [preparedStatement setBoolean:1:@"_pf":self->_pending];    		
    [preparedStatement setLong:2:@"g":self.surrogateKey];
	return preparedStatement;
}
...
- (id<SUPStatementWrapper>)getUpdateStatement:(id<SUPConnectionWrapper>)connection forTable:(SUPEntityTable)table;
{
    ...
    if (table == MainEntityTable)
    {
    	preparedStatement = [connection prepareStatement:@"update rhm_comercial_1_0_ts_plantillas_pos set \"a\"=?,\"b\"=?,\"c\"=?,\"d\"=?,\"e\"=?,\"f\"=?,\"_pc\"=?,\"_rp\"=?,\"_rf\"=?,\"_rc\"=?,\"_ds\"=? where \"_pf\"=? and \"g\"=?"];
    }
    else
    {
        preparedStatement = [connection prepareStatement:@"update rhm_comercial_1_0_ts_plantillas_pos_os set \"a\"=?,\"b\"=?,\"c\"=?,\"d\"=?,\"e\"=?,\"f\"=?,\"_pc\"=?,\"_rp\"=?,\"_rf\"=?,\"_rc\"=?,\"_ds\"=? where \"_pf\"=? and \"g\"=?"];
    }
    // Add updated values
    int parameterIndex = 0;
	if(self.BUKRS == nil)
		self.BUKRS = @"";
		//RHM_Comercial_TS_PLANTILLAS_POS getUpdateStatement:BUKRS not allowed to be null
    parameterIndex++;
    [preparedStatement setString:parameterIndex:@"a":self.BUKRS];
	if(self.PLANTILLA == nil)
		self.PLANTILLA = @"";
		//RHM_Comercial_TS_PLANTILLAS_POS getUpdateStatement:PLANTILLA not allowed to be null
    parameterIndex++;
    [preparedStatement setString:parameterIndex:@"b":self.PLANTILLA];
	if(self.POSICION == nil)
		self.POSICION = @"";
		//RHM_Comercial_TS_PLANTILLAS_POS getUpdateStatement:POSICION not allowed to be null
    parameterIndex++;
    [preparedStatement setString:parameterIndex:@"c":self.POSICION];
	if(self.MATNR == nil)
		self.MATNR = @"";
		//RHM_Comercial_TS_PLANTILLAS_POS getUpdateStatement:MATNR not allowed to be null
    parameterIndex++;
    [preparedStatement setString:parameterIndex:@"d":self.MATNR];
	if(self.CANTIDAD == nil)
		self.CANTIDAD = [NSNumber numberWithFloat:0.0];
		//RHM_Comercial_TS_PLANTILLAS_POS getUpdateStatement:CANTIDAD not allowed to be null
    parameterIndex++;
    [preparedStatement setDecimal:parameterIndex:@"e":self.CANTIDAD];
	if(self.UNIDAD == nil)
		self.UNIDAD = @"";
		//RHM_Comercial_TS_PLANTILLAS_POS getUpdateStatement:UNIDAD not allowed to be null
    parameterIndex++;
    [preparedStatement setString:parameterIndex:@"f":self.UNIDAD];
    parameterIndex++;
    ...
    parameterIndex++;
    [preparedStatement setLong:parameterIndex:@"g":self.surrogateKey];
	return preparedStatement;
}
...
+ (void)importData:(RHM_Comercial_TS_PLANTILLAS_POS*)serverEntity:(SUPJsonObject*)jsonObject
{
	...
	RHM_Comercial_TS_PLANTILLAS_POS* serverEntity2 = [RHM_Comercial_TS_PLANTILLAS_POS getInstance];
    ...
    if([RHM_Comercial_TS_PLANTILLAS_POS callbackHandler])
		[[RHM_Comercial_TS_PLANTILLAS_POS callbackHandler] beforeImport:serverEntity2];
    ...
    } else if(delete)
    {
        ...
        if(isCreated) 
        {
            RHM_Comercial_TS_PLANTILLAS_POS *clientEntity = [RHM_Comercial_TS_PLANTILLAS_POS find:[serverEntity _pk]];
	...
	if(update)
    {
    	...
    	if(clientEntity.pending)
    		clientEntity = (RHM_Comercial_TS_PLANTILLAS_POS*)clientEntity.downloadState;
        @try 
        {
    ...
    if([RHM_Comercial_TS_PLANTILLAS_POS callbackHandler])
		[[RHM_Comercial_TS_PLANTILLAS_POS callbackHandler] onImport:serverEntity];
...
+ (RHM_Comercial_TS_PLANTILLAS_POS*)load:(int64_t)id
{
	RHM_Comercial_TS_PLANTILLAS_POS* entity = [self find:id inTable:MainEntityTable];
	//RHM_Comercial_TS_PLANTILLAS_POS* entity = [self find:id];
    if (entity == nil)
    {
		@throw [SUPObjectNotFoundException withMessage:@"-load: in RHM_Comercial_TS_PLANTILLAS_POS: entity not found in database"];
    }
    return entity;
}

static SUPEntityMetaData* RHM_Comercial_TS_PLANTILLAS_POS_META_DATA;
...
+ (SUPEntityMetaData*)metaData
{
    if (! (_valid_))
    {
       RHM_Comercial_TS_PLANTILLAS_POS_META_DATA = [SUPEntityMetaData getInstance];
       RHM_Comercial_TS_PLANTILLAS_POS_META_DATA.id = _ENTITY_NUMBER_FOR_RHM_Comercial_TS_PLANTILLAS_POS_;
       RHM_Comercial_TS_PLANTILLAS_POS_META_DATA.name = @"TS_PLANTILLAS_POS";
       [RHM_Comercial_TS_PLANTILLAS_POS_META_DATA setClass:[RHM_Comercial_TS_PLANTILLAS_POS class]];

	   RHM_Comercial_TS_PLANTILLAS_POS_META_DATA.isClientOnly = NO;
       ...
       // Handle attributes
		...
		[a_BUKRS setScale:0];
		[a_BUKRS setIsKey:NO];
		SUPAttributeMetaData_DC* a_PLANTILLA = [RHM_Comercial_RHM_ComercialDB createAttributeMetaData:2:(SUPString)@"PLANTILLA":[SUPDataType forName:@"string"]];
		[a_PLANTILLA setColumn:@"\"b\""];
		[a_PLANTILLA setMaxLength:6];
		[a_PLANTILLA setPrecision:0];
		[a_PLANTILLA setScale:0];
		[a_PLANTILLA setIsKey:NO];
		SUPAttributeMetaData_DC* a_POSICION = [RHM_Comercial_RHM_ComercialDB createAttributeMetaData:3:(SUPString)@"POSICION":[SUPDataType forName:@"string"]];
		[a_POSICION setColumn:@"\"c\""];
		[a_POSICION setMaxLength:6];
		[a_POSICION setPrecision:0];
		[a_POSICION setScale:0];
		[a_POSICION setIsKey:NO];
		SUPAttributeMetaData_DC* a_MATNR = [RHM_Comercial_RHM_ComercialDB createAttributeMetaData:4:(SUPString)@"MATNR":[SUPDataType forName:@"string"]];
		[a_MATNR setColumn:@"\"d\""];
		[a_MATNR setMaxLength:18];
		[a_MATNR setPrecision:0];
		[a_MATNR setScale:0];
		[a_MATNR setIsKey:NO];
		SUPAttributeMetaData_DC* a_CANTIDAD = [RHM_Comercial_RHM_ComercialDB createAttributeMetaData:5:(SUPString)@"CANTIDAD":[SUPDataType forName:@"decimal"]];
		[a_CANTIDAD setColumn:@"\"e\""];
		[a_CANTIDAD setMaxLength:-1];
		[a_CANTIDAD setPrecision:28];
		[a_CANTIDAD setScale:8];
		[a_CANTIDAD setIsKey:NO];
		SUPAttributeMetaData_DC* a_UNIDAD = [RHM_Comercial_RHM_ComercialDB createAttributeMetaData:6:(SUPString)@"UNIDAD":[SUPDataType forName:@"string"]];
		[a_UNIDAD setColumn:@"\"f\""];
		[a_UNIDAD setMaxLength:3];
		[a_UNIDAD setPrecision:0];
		[a_UNIDAD setScale:0];
		[a_UNIDAD setIsKey:NO];
		SUPAttributeMetaData_DC* a_pending = [RHM_Comercial_RHM_ComercialDB createAttributeMetaData:7:(SUPString)@"pending":[SUPDataType forName:@"boolean"]];
		[a_pending setColumn:@"\"_pf\""];
		[a_pending setMaxLength:-1];
		[a_pending setPrecision:0];
		[a_pending setScale:0];
		[a_pending setIsKey:NO];
		SUPAttributeMetaData_DC* a_pendingChange = [RHM_Comercial_RHM_ComercialDB createAttributeMetaData:8:(SUPString)@"pendingChange":[SUPDataType forName:@"char"]];
		...
		SUPAttributeMetaData_DC* a_replayPending = [RHM_Comercial_RHM_ComercialDB createAttributeMetaData:9:(SUPString)@"replayPending":[SUPDataType forName:@"long"]];
		...
		SUPAttributeMetaData_DC* a_replayFailure = [RHM_Comercial_RHM_ComercialDB createAttributeMetaData:10:(SUPString)@"replayFailure":[SUPDataType forName:@"long"]];
		...
		SUPAttributeMetaData_DC* a_surrogateKey = [RHM_Comercial_RHM_ComercialDB createAttributeMetaData:11:(SUPString)@"surrogateKey":[SUPDataType forName:@"long"]];
		...
		SUPAttributeMetaData_DC* a_replayCounter = [RHM_Comercial_RHM_ComercialDB createAttributeMetaData:12:(SUPString)@"replayCounter":[SUPDataType forName:@"long"]];
		...
		SUPAttributeMetaData_DC* a_disableSubmit = [RHM_Comercial_RHM_ComercialDB createAttributeMetaData:13:(SUPString)@"disableSubmit":[SUPDataType forName:@"boolean"]];
		...
		[a_disableSubmit setIsKey:NO];
 		...
 		[attributes addThis:a_PLANTILLA];
 		[attributes addThis:a_POSICION];
 		[attributes addThis:a_MATNR];
 		[attributes addThis:a_CANTIDAD];
 		[attributes addThis:a_UNIDAD];
 		...
 		RHM_Comercial_TS_PLANTILLAS_POS_META_DATA.attributes = attributes;
 		
 		SUPAttributeMap * attributeMap = [SUPAttributeMap getInstance];
    	[attributeMap setAttributes:attributes];
    	RHM_Comercial_TS_PLANTILLAS_POS_META_DATA.attributeMap = [attributeMap finishInit];
 
 		// Handle operations
 		SUPOperationMetaData* o_findAll_0 = [RHM_Comercial_RHM_ComercialDB createOperationMetaData:1:(SUPString)@"findAll":[SUPDataType forName:@"TS_PLANTILLAS_POS*"]:true];
		...
		[o_findAll_0 setIsDelete:NO]; 		
 		...
 		SUPOperationMetaData* o_getPendingObjects_0 = [RHM_Comercial_RHM_ComercialDB createOperationMetaData:2:(SUPString)@"getPendingObjects":[SUPDataType forName:@"TS_PLANTILLAS_POS*"]:true];
		...
		[o_getPendingObjects_0 setIsDelete:NO]; 		
 		...
 		SUPOperationMetaData* o_getPendingObjects_1 = [RHM_Comercial_RHM_ComercialDB createOperationMetaData:3:(SUPString)@"getPendingObjects":[SUPDataType forName:@"TS_PLANTILLAS_POS*"]:true];
  		{
		...
		[o_getPendingObjects_1 setIsDelete:NO]; 		
 		...
 		SUPOperationMetaData* o_getReplayPendingObjects_0 = [RHM_Comercial_RHM_ComercialDB createOperationMetaData:4:(SUPString)@"getReplayPendingObjects":[SUPDataType forName:@"TS_PLANTILLAS_POS*"]:true];
		...
		[o_save_0 setIsDelete:NO]; 		
 		[o_save_0 finishInit];
 		SUPOperationMetaData* o_findWithQuery_1 = [RHM_Comercial_RHM_ComercialDB createOperationMetaData:12:(SUPString)@"findWithQuery":[SUPDataType forName:@"TS_PLANTILLAS_POS*"]:true];
  		{
 			SUPObjectList *parameters_list = nil;
		...
		[o_countReplayPendingObjects_0 setIsDelete:NO]; 		
 		...
 		RHM_Comercial_TS_PLANTILLAS_POS_META_DATA.operations = operations;
 		
		SUPOperationMap *operationMap = [SUPOperationMap getInstance];
		[operationMap setOperations:operations];
		RHM_Comercial_TS_PLANTILLAS_POS_META_DATA.operationMap = [operationMap finishInit];
		
		RHM_Comercial_TS_PLANTILLAS_POS_META_DATA.table = @"\"rhm_comercial_1_0_ts_plantillas_pos\"";

 		[RHM_Comercial_TS_PLANTILLAS_POS_META_DATA retain];
     	_valid_ = YES;

	}
	return RHM_Comercial_TS_PLANTILLAS_POS_META_DATA;
}
...
- (void)merge:(RHM_Comercial_TS_PLANTILLAS_POS*)entity withJson:(SUPJsonObject*)json
{
    ...
    if(![json containsKey:@"PLANTILLA"] )
    {
    	self.PLANTILLA = entity.PLANTILLA;
    }
    else if(![json getString:@"PLANTILLA"])
    {
    	self.PLANTILLA = entity.PLANTILLA;
    }
    if(![json containsKey:@"POSICION"] )
    {
    	self.POSICION = entity.POSICION;
    }
    else if(![json getString:@"POSICION"])
    {
    	self.POSICION = entity.POSICION;
    }
    if(![json containsKey:@"MATNR"] )
    ...
    if(![json containsKey:@"CANTIDAD"] )
    {
    	self.CANTIDAD = entity.CANTIDAD;
    }
    else if(![json getDecimal:@"CANTIDAD"])
    {
    	self.CANTIDAD = entity.CANTIDAD;
    }
    if(![json containsKey:@"UNIDAD"] )
    {
    	self.UNIDAD = entity.UNIDAD;
    }
    else if(![json getString:@"UNIDAD"])
    {
    	self.UNIDAD = entity.UNIDAD;
    }
    if(![json containsKey:@"surrogateKey"] )
...
+ (RHM_Comercial_TS_PLANTILLAS_POS*)getInstance
{
    RHM_Comercial_TS_PLANTILLAS_POS* me = [[RHM_Comercial_TS_PLANTILLAS_POS alloc] init];
    [me autorelease];
    return me;
}
...
- (SUPObjectList*)getLogRecords
{
    ...
    query.testCriteria = (SUPTestCriteria*)[[SUPAttributeTest match:@"component":@"TS_PLANTILLAS_POS"] and:[SUPAttributeTest match:@"entityKey":[self keyToString]]];
...
{
    {
        ...
        log.component = @"TS_PLANTILLAS_POS";
...
- (void)refresh
{
    if (!self.isNew)
    {
        RHM_Comercial_TS_PLANTILLAS_POS* entity = [RHM_Comercial_TS_PLANTILLAS_POS load:[self id]];
        [self copyAll:entity];
	...
	for (id item in content)
    {
        ...
        if ([headers containsKey:@"log"]) 
        {
            for(SUPJsonObject *o in [headers getArray:@"log"])
			{
				...
				[RHM_Comercial_RHM_ComercialDB writeLogFromHeader:o:@"TS_PLANTILLAS_POS":@"replay"];
			}
        }

        RHM_Comercial_TS_PLANTILLAS_POS* clientEntity = [RHM_Comercial_TS_PLANTILLAS_POS find:key];
        if( (clientEntity != nil) && (clientEntity.pending) )
        {
            ...
            if(client_rc == server_id)
            {
				...
				if([RHM_Comercial_TS_PLANTILLAS_POS callbackHandler])
	        		[[RHM_Comercial_TS_PLANTILLAS_POS callbackHandler] onReplaySuccess:clientEntity];       
...
+ (void)replayRejected:(SUPJsonObject*)headers withContent:(SUPJsonArray*)content
{
    for (id item in content)
    {
        ...
        if(clientEntity != nil)
        {
            ...
            if(client_rc == server_id)
            {
	        	RHM_Comercial_TS_PLANTILLAS_POS* originalState = (RHM_Comercial_TS_PLANTILLAS_POS*)clientEntity.originalState;
	        	SUPChar pendingChange = clientEntity.pendingChange;
				...
				clientEntity->_isDeleted = (pendingChange == PendingDelete);
			    if([RHM_Comercial_TS_PLANTILLAS_POS callbackHandler])
			    	[[RHM_Comercial_TS_PLANTILLAS_POS callbackHandler] onReplayFailure:clientEntity];
		        if([RHM_Comercial_RHM_ComercialDB callbackHandler])
...
-(void)submitPending1:(BOOL)doRefresh
{
    ...
    @try
    {
        ...
        if (self.pending && !self.disableSubmit && (self.replayCounter > self.replayPending))
        {
            ...
            [headers setString:@"mbo":@"TS_PLANTILLAS_POS"];
    ...
    @finally
    {
        ...
        [[RHM_Comercial_RHM_ComercialDB dblock] unlock];
        if(success)
            [RHM_Comercial_RHM_ComercialDB replay:@"TS_PLANTILLAS_POS" withId:self.replayCounter withContent:[message content]];
        [message release];
    }
}
...
- (void)createPending:(BOOL)isPending
{
    ...
    @try
    {
		...
		if (isPending)
		{
			RHM_Comercial_TS_PLANTILLAS_POS* originalEntity = (RHM_Comercial_TS_PLANTILLAS_POS*)[self originalState];
...
- (SUPAbstractEntity*)originalState
{
    ...
    {
        self->_originalState = self.pending ? [RHM_Comercial_TS_PLANTILLAS_POS find:[self id] inTable:OriginalStateEntityTable] : nil;
...
- (SUPAbstractEntity*)downloadState
{
    if (!self->_downloadStateValid)
    {
        ...
        {
            RHM_Comercial_TS_PLANTILLAS_POS *mbo = [RHM_Comercial_TS_PLANTILLAS_POS find:[self id]];
            if(mbo)
...
- (void)updateOriginalState
{
    ...
    @try
    {
		...
		RHM_Comercial_TS_PLANTILLAS_POS* os = [RHM_Comercial_TS_PLANTILLAS_POS find:[self id] inTable:OriginalStateEntityTable];
		if(os)
		{
...
+ (SUPObjectList*)findAll:(int32_t)skip take:(int32_t)take
{
	...
	@try
	{
	    [[RHM_Comercial_RHM_ComercialDB dblock] readLock];
		connection = [RHM_Comercial_RHM_ComercialDB getConnectionWrapper];
		sql = [[NSMutableString alloc] initWithCapacity:320];
	    [sql appendString:@"SELECT x.\"a\",x.\"b\",x.\"c\",x.\"d\",x.\"e\",x.\"f\",x.\"_pf\",x.\"_pc\",x.\"_rp\",x.\"_rf\",x.\"g\",x.\"_rc\",x.\"_ds\" FROM \"rhm_comercial_1_0_ts_plantillas_pos\" x where (((x.\"_pf\" = 1 or not exists (select x_os.\"g\" from \"rhm_comercial_1_0_ts_plantillas_pos_os\" x_os where x_os.\"g\" = x.\"g\")))"
	                            ")"];
	    if(take != -1)
...
+ (SUPObjectList*)getPendingObjects:(int32_t)skip take:(int32_t)take
{
	...
	@try
	{
	    ...
	    [sql appendString:@"select x.\"a\",x.\"b\",x.\"c\",x.\"d\",x.\"e\",x.\"f\",x.\"_pf\",x.\"_pc\",x.\"_rp\",x.\"_rf\",x.\"g\",x.\"_rc\",x.\"_ds\" from \"rhm_comercial_1_0_ts_plantillas_pos\" x where x.\"_pc\" <> \'N\' and x.\"_rc\" != x.\"_rp\""];
	    if(take != -1)
	        [sql appendFormat:@" limit %i",take];
	    if(skip != 0)
	        [sql appendFormat:@" offset %i",skip];
...
+ (SUPObjectList*)getPendingObjects:(unichar)pendingChange skip:(int32_t)skip take:(int32_t)take
{
	...
	@try
	{
	    ...
	    [sql appendString:@"select x.\"a\",x.\"b\",x.\"c\",x.\"d\",x.\"e\",x.\"f\",x.\"_pf\",x.\"_pc\",x.\"_rp\",x.\"_rf\",x.\"g\",x.\"_rc\",x.\"_ds\" from \"rhm_comercial_1_0_ts_plantillas_pos\" x where x.\"_pc\" = ? and x.\"_rc\" != x.\"_rp\""];
	    if(take != -1)
	        [sql appendFormat:@" limit %i",take];
	    if(skip != 0)
	        [sql appendFormat:@" offset %i",skip];
...
{
    return [RHM_Comercial_TS_PLANTILLAS_POS countWithQuery:query];
}
...
+ (int32_t)countReplayPendingObjects
{
	...
	@try
	{
	    [[RHM_Comercial_RHM_ComercialDB dblock] readLock];
		connection = [RHM_Comercial_RHM_ComercialDB getConnectionWrapper];
		sql = [[NSMutableString alloc] initWithCapacity:235];
	    [sql appendString:@"select count(*) ,x.\"g\" from \"rhm_comercial_1_0_ts_plantillas_pos\" x where (((x.\"_pf\" = 1 or not exists (select x_os.\"g\" from \"rhm_comercial_1_0_ts_plantillas_pos_os\" x_os where x_os.\"g\" = x.\"g\")))) and ( x.\"_rp\" != 0)"];
