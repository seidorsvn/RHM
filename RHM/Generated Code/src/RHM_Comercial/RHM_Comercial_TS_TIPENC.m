...
@interface RHM_Comercial_TS_TIPENC(private)
- (void)bind:(id<SUPResultSetWrapper>)resultSet;
- (void)bindWithQuery:(id<SUPResultSetWrapper>)resultSet:(SUPQuery*)query;
+ (RHM_Comercial_TS_TIPENC*)find:(int64_t)id inTable:(SUPEntityTable)table;
+ (void)importData:(RHM_Comercial_TS_TIPENC*)serverEntity:(SUPJsonObject*)jsonObject;
- (void)merge:(RHM_Comercial_TS_TIPENC*)entity withJson:(SUPJsonObject*)json;
- (void)setAllAttributesFrom:(RHM_Comercial_TS_TIPENC*)other;
@end
@implementation RHM_Comercial_TS_TIPENC

@synthesize BUKRS = _BUKRS;
@synthesize ZTIPENC = _ZTIPENC;
@synthesize ZDESCRIP = _ZDESCRIP;
...
- (void)setZTIPENC:(NSString*)newZTIPENC
{
    if (newZTIPENC != self->_ZTIPENC)
    {
		[self->_ZTIPENC release];
        self->_ZTIPENC = [newZTIPENC retain];
        self->_isDirty = YES;
    }
}

- (void)setZDESCRIP:(NSString*)newZDESCRIP
{
    if (newZDESCRIP != self->_ZDESCRIP)
    {
		[self->_ZDESCRIP release];
        self->_ZDESCRIP = [newZDESCRIP retain];
        self->_isDirty = YES;
    }
}
...
- (void)readJson:(SUPJsonObject*)json:(int32_t)flags
{
        if ([json containsKey:@"BUKRS"])
    {
        ...
        if ([json containsKey:@"ZTIPENC"])
    {
        self.ZTIPENC = [json getString:@"ZTIPENC"];
    }
        if ([json containsKey:@"ZDESCRIP"])
    {
        self.ZDESCRIP = [json getString:@"ZDESCRIP"];
    }
...
- (SUPJsonObject*)json:(int32_t)flags
{
	...
	if (all)
	{
	    ...
	    [newJson setString:@"ZTIPENC":self.ZTIPENC];
	    [newJson setString:@"ZDESCRIP":self.ZDESCRIP];
	    [newJson setBoolean:@"disableSubmit":self.disableSubmit];
	}
    [newJson setString:@"?":@"TS_TIPENC"];
    [newJson autorelease];
    return newJson;
}
+ (SUPObjectList*)fromJSONList:(SUPJsonArray*)jsonArray
{
	...
	{
	    RHM_Comercial_TS_TIPENC* inst = [[RHM_Comercial_TS_TIPENC alloc] initFromJson:jsonObject:0];
...
+ (SUPJsonArray*)toJSONList:(SUPObjectList*)instanceList
{
	...
	for (RHM_Comercial_TS_TIPENC* inst in instanceList)
	{
...
- (void)bind:(id<SUPResultSetWrapper>)resultSet
{
    @try
    {
        ...
        self.ZTIPENC = [resultSet getString:2:@"b"];
        self.ZDESCRIP = [resultSet getString:3:@"c"];
...
- (void)bindWithQuery:(id<SUPResultSetWrapper>)resultSet:(SUPQuery*)query
{
    ...
    SUPEntityMetaData *entity = [RHM_Comercial_TS_TIPENC metaData];
    ...
    for(int j=0;j<[cl size];j++)
    {
		...
		if([amd.name isEqualToString:@"ZTIPENC"])
        	self.ZTIPENC = [resultSet getString:j+1:@"b"];
		if([amd.name isEqualToString:@"ZDESCRIP"])
        	self.ZDESCRIP = [resultSet getString:j+1:@"c"];
...
- (void)copyAll:(RHM_Comercial_TS_TIPENC*)other
{
	[self copyAll:other withParent:(SUPAbstractEntity*)other];
}


- (void)copyAll:(RHM_Comercial_TS_TIPENC*)other withParent:(SUPAbstractEntity*)parent
{
    [self setBUKRS:other.BUKRS];
    
    [self setZTIPENC:other.ZTIPENC];
    
    [self setZDESCRIP:other.ZDESCRIP];
...
id<SUPResultSetWrapper> resultSet = nil;
    {
        ...
        {    
            ...
            statement = [SUPStatementBuilder prepareQuery:connection:[RHM_Comercial_RHM_ComercialDB metaData]:[RHM_Comercial_TS_TIPENC metaData]:query];
...
+ (RHM_Comercial_TS_TIPENC*)find_by_id_string:(NSString*)id_string
{
    if(id_string == nil) return nil;
	int64_t key;
	NSNumberFormatter *nf = [[[NSNumberFormatter alloc] init] autorelease];
	key = [[nf numberFromString:id_string] longValue];
    return [RHM_Comercial_TS_TIPENC find:key];
	
}

+ (RHM_Comercial_TS_TIPENC*)find:(int64_t)id
{
	return [self find:id inTable:MainEntityTable];
}

+ (RHM_Comercial_TS_TIPENC*)find:(int64_t)id_ inTable:(SUPEntityTable)table
{
    RHM_Comercial_TS_TIPENC* result = nil;
	RHM_Comercial_TS_TIPENC* downloadState = nil;
    ...
    @try
    {
    	...
    	//@synchronized(connection)
    	{
        	@try
        	{
            	...
            	{
                	preparedStatement = [connection prepareStatement:@"select \"a\",\"b\",\"c\",\"_pf\",\"_pc\",\"_rp\",\"_rf\",\"d\",\"_rc\",\"_ds\" from rhm_comercial_1_0_ts_tipenc where \"d\"=? order by \"_pf\""];
            	}
            	else
            	{
                	preparedStatement = [connection prepareStatement:@"select \"a\",\"b\",\"c\",\"_pf\",\"_pc\",\"_rp\",\"_rf\",\"d\",\"_rc\",\"_ds\" from rhm_comercial_1_0_ts_tipenc_os where \"d\"=? order by \"_pf\""];
            	}
            	...
            	while ([resultSet next])
            	{
                	...
                	result = [[RHM_Comercial_TS_TIPENC alloc] init];
                	...
                	{
 	              		downloadState = [RHM_Comercial_TS_TIPENC getInstance];
...
id<SUPResultSetWrapper> resultSet = nil;
    {
        ...
        {    
            ...
            connection = [RHM_Comercial_RHM_ComercialDB getConnectionWrapper];
            int32_t count = 0;
            RHM_Comercial_TS_TIPENCList* list = [[RHM_Comercial_TS_TIPENCList alloc] init];
            ...
            {
                count++;   
                RHM_Comercial_TS_TIPENC* entity = [[RHM_Comercial_TS_TIPENC alloc] init];
...
- (id<SUPStatementWrapper>)getCreateStatement:(id<SUPConnectionWrapper>)connection forTable:(SUPEntityTable)table;
{
    ...
    if (table == MainEntityTable)
    {
        preparedStatement = [connection prepareStatement:@"insert into rhm_comercial_1_0_ts_tipenc (\"a\",\"b\",\"c\",\"_pf\",\"_pc\",\"_rp\",\"_rf\",\"d\",\"_rc\",\"_ds\") values (?,?,?,?,?,?,?,?,?,?)"];
    }
    else
    {
        preparedStatement = [connection prepareStatement:@"insert into rhm_comercial_1_0_ts_tipenc_os (\"a\",\"b\",\"c\",\"_pf\",\"_pc\",\"_rp\",\"_rf\",\"d\",\"_rc\",\"_ds\") values (?,?,?,?,?,?,?,?,?,?)"];
    }
    // Add updated values
	if(self.BUKRS == nil)
		self.BUKRS = @"";
		//RHM_Comercial_TS_TIPENC getCreateStatement:BUKRS not allowed to be null
    [preparedStatement setString:1:@"a":self.BUKRS];
	if(self.ZTIPENC == nil)
		self.ZTIPENC = @"";
		//RHM_Comercial_TS_TIPENC getCreateStatement:ZTIPENC not allowed to be null
    [preparedStatement setString:2:@"b":self.ZTIPENC];
	if(self.ZDESCRIP == nil)
		self.ZDESCRIP = @"";
		//RHM_Comercial_TS_TIPENC getCreateStatement:ZDESCRIP not allowed to be null
    [preparedStatement setString:3:@"c":self.ZDESCRIP];
...
- (id<SUPStatementWrapper>)getDeleteStatement:(id<SUPConnectionWrapper>)connection forTable:(SUPEntityTable)table;
{
    ...
    if (table == MainEntityTable)
    {
        preparedStatement = [connection prepareStatement:@"delete from rhm_comercial_1_0_ts_tipenc where \"_pf\"=? and \"d\"=?"];
    }
    else
    {
        preparedStatement = [connection prepareStatement:@"delete from rhm_comercial_1_0_ts_tipenc_os where \"_pf\"=? and \"d\"=?"];
    }
...
- (id<SUPStatementWrapper>)getUpdateStatement:(id<SUPConnectionWrapper>)connection forTable:(SUPEntityTable)table;
{
    ...
    if (table == MainEntityTable)
    {
    	preparedStatement = [connection prepareStatement:@"update rhm_comercial_1_0_ts_tipenc set \"a\"=?,\"b\"=?,\"c\"=?,\"_pc\"=?,\"_rp\"=?,\"_rf\"=?,\"_rc\"=?,\"_ds\"=? where \"_pf\"=? and \"d\"=?"];
    }
    else
    {
        preparedStatement = [connection prepareStatement:@"update rhm_comercial_1_0_ts_tipenc_os set \"a\"=?,\"b\"=?,\"c\"=?,\"_pc\"=?,\"_rp\"=?,\"_rf\"=?,\"_rc\"=?,\"_ds\"=? where \"_pf\"=? and \"d\"=?"];
    }
    // Add updated values
    int parameterIndex = 0;
	if(self.BUKRS == nil)
		self.BUKRS = @"";
		//RHM_Comercial_TS_TIPENC getUpdateStatement:BUKRS not allowed to be null
    parameterIndex++;
    [preparedStatement setString:parameterIndex:@"a":self.BUKRS];
	if(self.ZTIPENC == nil)
		self.ZTIPENC = @"";
		//RHM_Comercial_TS_TIPENC getUpdateStatement:ZTIPENC not allowed to be null
    parameterIndex++;
    [preparedStatement setString:parameterIndex:@"b":self.ZTIPENC];
	if(self.ZDESCRIP == nil)
		self.ZDESCRIP = @"";
		//RHM_Comercial_TS_TIPENC getUpdateStatement:ZDESCRIP not allowed to be null
    parameterIndex++;
    [preparedStatement setString:parameterIndex:@"c":self.ZDESCRIP];
    parameterIndex++;
...
+ (void)importData:(RHM_Comercial_TS_TIPENC*)serverEntity:(SUPJsonObject*)jsonObject
{
	...
	RHM_Comercial_TS_TIPENC* serverEntity2 = [RHM_Comercial_TS_TIPENC getInstance];
    ...
    if([RHM_Comercial_TS_TIPENC callbackHandler])
		[[RHM_Comercial_TS_TIPENC callbackHandler] beforeImport:serverEntity2];
    ...
    } else if(delete)
    {
        ...
        if(isCreated) 
        {
            RHM_Comercial_TS_TIPENC *clientEntity = [RHM_Comercial_TS_TIPENC find:[serverEntity _pk]];
	...
	if(update)
    {
    	...
    	if(clientEntity.pending)
    		clientEntity = (RHM_Comercial_TS_TIPENC*)clientEntity.downloadState;
        @try 
        {
    ...
    if([RHM_Comercial_TS_TIPENC callbackHandler])
		[[RHM_Comercial_TS_TIPENC callbackHandler] onImport:serverEntity];
...
+ (RHM_Comercial_TS_TIPENC*)load:(int64_t)id
{
	RHM_Comercial_TS_TIPENC* entity = [self find:id inTable:MainEntityTable];
	//RHM_Comercial_TS_TIPENC* entity = [self find:id];
    if (entity == nil)
    {
		@throw [SUPObjectNotFoundException withMessage:@"-load: in RHM_Comercial_TS_TIPENC: entity not found in database"];
    }
    return entity;
}

static SUPEntityMetaData* RHM_Comercial_TS_TIPENC_META_DATA;
...
+ (SUPEntityMetaData*)metaData
{
    if (! (_valid_))
    {
       RHM_Comercial_TS_TIPENC_META_DATA = [SUPEntityMetaData getInstance];
       RHM_Comercial_TS_TIPENC_META_DATA.id = _ENTITY_NUMBER_FOR_RHM_Comercial_TS_TIPENC_;
       RHM_Comercial_TS_TIPENC_META_DATA.name = @"TS_TIPENC";
       [RHM_Comercial_TS_TIPENC_META_DATA setClass:[RHM_Comercial_TS_TIPENC class]];

	   RHM_Comercial_TS_TIPENC_META_DATA.isClientOnly = NO;
       ...
       // Handle attributes
		...
		[a_BUKRS setScale:0];
		[a_BUKRS setIsKey:NO];
		SUPAttributeMetaData_DC* a_ZTIPENC = [RHM_Comercial_RHM_ComercialDB createAttributeMetaData:2:(SUPString)@"ZTIPENC":[SUPDataType forName:@"string"]];
		[a_ZTIPENC setColumn:@"\"b\""];
		[a_ZTIPENC setMaxLength:1];
		[a_ZTIPENC setPrecision:0];
		[a_ZTIPENC setScale:0];
		[a_ZTIPENC setIsKey:NO];
		SUPAttributeMetaData_DC* a_ZDESCRIP = [RHM_Comercial_RHM_ComercialDB createAttributeMetaData:3:(SUPString)@"ZDESCRIP":[SUPDataType forName:@"string"]];
		[a_ZDESCRIP setColumn:@"\"c\""];
		[a_ZDESCRIP setMaxLength:30];
		[a_ZDESCRIP setPrecision:0];
		[a_ZDESCRIP setScale:0];
		[a_ZDESCRIP setIsKey:NO];
		...
		[a_disableSubmit setIsKey:NO];
 		...
 		[attributes addThis:a_ZTIPENC];
 		[attributes addThis:a_ZDESCRIP];
 		...
 		RHM_Comercial_TS_TIPENC_META_DATA.attributes = attributes;
 		
 		SUPAttributeMap * attributeMap = [SUPAttributeMap getInstance];
    	[attributeMap setAttributes:attributes];
    	RHM_Comercial_TS_TIPENC_META_DATA.attributeMap = [attributeMap finishInit];
 
 		// Handle operations
 		SUPOperationMetaData* o_findAll_0 = [RHM_Comercial_RHM_ComercialDB createOperationMetaData:1:(SUPString)@"findAll":[SUPDataType forName:@"TS_TIPENC*"]:true];
		...
		[o_findAll_0 setIsDelete:NO]; 		
 		...
 		SUPOperationMetaData* o_getPendingObjects_0 = [RHM_Comercial_RHM_ComercialDB createOperationMetaData:2:(SUPString)@"getPendingObjects":[SUPDataType forName:@"TS_TIPENC*"]:true];
		...
		[o_getPendingObjects_0 setIsDelete:NO]; 		
 		...
 		SUPOperationMetaData* o_getPendingObjects_1 = [RHM_Comercial_RHM_ComercialDB createOperationMetaData:3:(SUPString)@"getPendingObjects":[SUPDataType forName:@"TS_TIPENC*"]:true];
  		{
		...
		[o_getPendingObjects_1 setIsDelete:NO]; 		
 		...
 		SUPOperationMetaData* o_getReplayPendingObjects_0 = [RHM_Comercial_RHM_ComercialDB createOperationMetaData:4:(SUPString)@"getReplayPendingObjects":[SUPDataType forName:@"TS_TIPENC*"]:true];
		...
		[o_save_0 setIsDelete:NO]; 		
 		[o_save_0 finishInit];
 		SUPOperationMetaData* o_findWithQuery_1 = [RHM_Comercial_RHM_ComercialDB createOperationMetaData:12:(SUPString)@"findWithQuery":[SUPDataType forName:@"TS_TIPENC*"]:true];
  		{
 			SUPObjectList *parameters_list = nil;
		...
		[o_countReplayPendingObjects_0 setIsDelete:NO]; 		
 		...
 		RHM_Comercial_TS_TIPENC_META_DATA.operations = operations;
 		
		SUPOperationMap *operationMap = [SUPOperationMap getInstance];
		[operationMap setOperations:operations];
		RHM_Comercial_TS_TIPENC_META_DATA.operationMap = [operationMap finishInit];
		
		RHM_Comercial_TS_TIPENC_META_DATA.table = @"\"rhm_comercial_1_0_ts_tipenc\"";

 		[RHM_Comercial_TS_TIPENC_META_DATA retain];
     	_valid_ = YES;

	}
	return RHM_Comercial_TS_TIPENC_META_DATA;
}
...
- (void)merge:(RHM_Comercial_TS_TIPENC*)entity withJson:(SUPJsonObject*)json
{
    ...
    if(![json containsKey:@"ZTIPENC"] )
    {
    	self.ZTIPENC = entity.ZTIPENC;
    }
    else if(![json getString:@"ZTIPENC"])
    {
    	self.ZTIPENC = entity.ZTIPENC;
    }
    if(![json containsKey:@"ZDESCRIP"] )
    {
    	self.ZDESCRIP = entity.ZDESCRIP;
    }
    else if(![json getString:@"ZDESCRIP"])
    {
    	self.ZDESCRIP = entity.ZDESCRIP;
    }
    if(![json containsKey:@"surrogateKey"] )
...
+ (RHM_Comercial_TS_TIPENC*)getInstance
{
    RHM_Comercial_TS_TIPENC* me = [[RHM_Comercial_TS_TIPENC alloc] init];
    [me autorelease];
    return me;
}
...
- (SUPObjectList*)getLogRecords
{
    ...
    query.testCriteria = (SUPTestCriteria*)[[SUPAttributeTest match:@"component":@"TS_TIPENC"] and:[SUPAttributeTest match:@"entityKey":[self keyToString]]];
...
- (void)refresh
{
    if (!self.isNew)
    {
        RHM_Comercial_TS_TIPENC* entity = [RHM_Comercial_TS_TIPENC load:[self id]];
        [self copyAll:entity];
	...
	for (id item in content)
    {
        ...
        if ([headers containsKey:@"log"]) 
        {
            for(SUPJsonObject *o in [headers getArray:@"log"])
			{
				...
				[RHM_Comercial_RHM_ComercialDB writeLogFromHeader:o:@"TS_TIPENC":@"replay"];
			}
        }

        RHM_Comercial_TS_TIPENC* clientEntity = [RHM_Comercial_TS_TIPENC find:key];
        if( (clientEntity != nil) && (clientEntity.pending) )
        {
            ...
            if(client_rc == server_id)
            {
				...
				if([RHM_Comercial_TS_TIPENC callbackHandler])
	        		[[RHM_Comercial_TS_TIPENC callbackHandler] onReplaySuccess:clientEntity];       
...
+ (void)replayRejected:(SUPJsonObject*)headers withContent:(SUPJsonArray*)content
{
    for (id item in content)
    {
        ...
        if(clientEntity != nil)
        {
            ...
            if(client_rc == server_id)
            {
	        	RHM_Comercial_TS_TIPENC* originalState = (RHM_Comercial_TS_TIPENC*)clientEntity.originalState;
	        	SUPChar pendingChange = clientEntity.pendingChange;
				...
				clientEntity->_isDeleted = (pendingChange == PendingDelete);
			    if([RHM_Comercial_TS_TIPENC callbackHandler])
			    	[[RHM_Comercial_TS_TIPENC callbackHandler] onReplayFailure:clientEntity];
		        if([RHM_Comercial_RHM_ComercialDB callbackHandler])
...
-(void)submitPending1:(BOOL)doRefresh
{
    ...
    @try
    {
        ...
        if (self.pending && !self.disableSubmit && (self.replayCounter > self.replayPending))
        {
            ...
            [headers setString:@"mbo":@"TS_TIPENC"];
    ...
    @finally
    {
        ...
        [[RHM_Comercial_RHM_ComercialDB dblock] unlock];
        if(success)
            [RHM_Comercial_RHM_ComercialDB replay:@"TS_TIPENC" withId:self.replayCounter withContent:[message content]];
        [message release];
    }
}
...
- (void)createPending:(BOOL)isPending
{
    ...
    @try
    {
		...
		if (isPending)
		{
			RHM_Comercial_TS_TIPENC* originalEntity = (RHM_Comercial_TS_TIPENC*)[self originalState];
...
- (SUPAbstractEntity*)originalState
{
    ...
    {
        self->_originalState = self.pending ? [RHM_Comercial_TS_TIPENC find:[self id] inTable:OriginalStateEntityTable] : nil;
...
- (SUPAbstractEntity*)downloadState
{
    if (!self->_downloadStateValid)
    {
        ...
        {
            RHM_Comercial_TS_TIPENC *mbo = [RHM_Comercial_TS_TIPENC find:[self id]];
            if(mbo)
...
- (void)updateOriginalState
{
    ...
    @try
    {
		...
		RHM_Comercial_TS_TIPENC* os = [RHM_Comercial_TS_TIPENC find:[self id] inTable:OriginalStateEntityTable];
		if(os)
		{
...
+ (SUPObjectList*)findAll:(int32_t)skip take:(int32_t)take
{
	...
	@try
	{
	    [[RHM_Comercial_RHM_ComercialDB dblock] readLock];
		connection = [RHM_Comercial_RHM_ComercialDB getConnectionWrapper];
		sql = [[NSMutableString alloc] initWithCapacity:271];
	    [sql appendString:@"SELECT x.\"a\",x.\"b\",x.\"c\",x.\"_pf\",x.\"_pc\",x.\"_rp\",x.\"_rf\",x.\"d\",x.\"_rc\",x.\"_ds\" FROM \"rhm_comercial_1_0_ts_tipenc\" x where (((x.\"_pf\" = 1 or not exists (select x_os.\"d\" from \"rhm_comercial_1_0_ts_tipenc_os\" x_os where x_os.\"d\" = x.\"d\"))))"];
	    if(take != -1)
...
+ (SUPObjectList*)getPendingObjects:(int32_t)skip take:(int32_t)take
{
	...
	@try
	{
	    [[RHM_Comercial_RHM_ComercialDB dblock] readLock];
		connection = [RHM_Comercial_RHM_ComercialDB getConnectionWrapper];
		sql = [[NSMutableString alloc] initWithCapacity:191];
	    [sql appendString:@"select x.\"a\",x.\"b\",x.\"c\",x.\"_pf\",x.\"_pc\",x.\"_rp\",x.\"_rf\",x.\"d\",x.\"_rc\",x.\"_ds\" from \"rhm_comercial_1_0_ts_tipenc\" x where x.\"_pc\" <> \'N\' and x.\"_rc\" != x.\"_rp\""];
	    if(take != -1)
	        [sql appendFormat:@" limit %i",take];
	    if(skip != 0)
	        [sql appendFormat:@" offset %i",skip];
...
+ (SUPObjectList*)getPendingObjects:(unichar)pendingChange skip:(int32_t)skip take:(int32_t)take
{
	...
	@try
	{
	    [[RHM_Comercial_RHM_ComercialDB dblock] readLock];
		connection = [RHM_Comercial_RHM_ComercialDB getConnectionWrapper];
		sql = [[NSMutableString alloc] initWithCapacity:186];
	    [sql appendString:@"select x.\"a\",x.\"b\",x.\"c\",x.\"_pf\",x.\"_pc\",x.\"_rp\",x.\"_rf\",x.\"d\",x.\"_rc\",x.\"_ds\" from \"rhm_comercial_1_0_ts_tipenc\" x where x.\"_pc\" = ? and x.\"_rc\" != x.\"_rp\""];
	    if(take != -1)
	        [sql appendFormat:@" limit %i",take];
	    if(skip != 0)
	        [sql appendFormat:@" offset %i",skip];
...
+ (SUPObjectList*)getReplayPendingObjects:(int32_t)skip take:(int32_t)take
{
	...
	@try
	{
	    [[RHM_Comercial_RHM_ComercialDB dblock] readLock];
		connection = [RHM_Comercial_RHM_ComercialDB getConnectionWrapper];
		sql = [[NSMutableString alloc] initWithCapacity:293];
	    [sql appendString:@"select x.\"a\",x.\"b\",x.\"c\",x.\"_pf\",x.\"_pc\",x.\"_rp\",x.\"_rf\",x.\"d\",x.\"_rc\",x.\"_ds\" from \"rhm_comercial_1_0_ts_tipenc\" x where (((x.\"_pf\" = 1 or not exists (select x_os.\"d\" from \"rhm_comercial_1_0_ts_tipenc_os\" x_os where x_os.\"d\" = x.\"d\")))) and ( x.\"_rp\" != 0)"];
	    if(take != -1)
	        [sql appendFormat:@" limit %i",take];
	    if(skip != 0)
	        [sql appendFormat:@" offset %i",skip];
...
{
    return [RHM_Comercial_TS_TIPENC countWithQuery:query];
}
...
+ (int32_t)countReplayPendingObjects
{
	...
	@try
	{
	    ...
	    [sql appendString:@"select count(*) ,x.\"d\" from \"rhm_comercial_1_0_ts_tipenc\" x where (((x.\"_pf\" = 1 or not exists (select x_os.\"d\" from \"rhm_comercial_1_0_ts_tipenc_os\" x_os where x_os.\"d\" = x.\"d\")))) and ( x.\"_rp\" != 0)"];
