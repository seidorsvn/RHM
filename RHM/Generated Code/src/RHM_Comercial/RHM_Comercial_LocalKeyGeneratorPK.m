...
@implementation RHM_Comercial_LocalKeyGeneratorPK
...
+ (SUPObjectList*)fromJSONList:(SUPJsonArray*)jsonArray
{
	...
	{
	    RHM_Comercial_LocalKeyGeneratorPK* inst = [[RHM_Comercial_LocalKeyGeneratorPK alloc] initFromJson:jsonObject:0];
...
+ (SUPJsonArray*)toJSONList:(SUPObjectList*)instanceList
{
	...
	for (RHM_Comercial_LocalKeyGeneratorPK* inst in instanceList)
	{
...
static SUPClassMetaData* RHM_Comercial_LocalKeyGeneratorPK_META_DATA;
...
+ (SUPClassMetaData*)metaData
{
    if (! (_valid_))
    {
 	   	RHM_Comercial_LocalKeyGeneratorPK_META_DATA = [[SUPEntityMetaData alloc] init];
 	   	SUPObjectList *attributes = [SUPObjectList listWithCapacity:7];
       	RHM_Comercial_LocalKeyGeneratorPK_META_DATA.name = @"LocalKeyGeneratorPK";
       	[RHM_Comercial_LocalKeyGeneratorPK_META_DATA setClass:[RHM_Comercial_LocalKeyGeneratorPK class]];
		...
		SUPAttributeMetaData_DC* a_batchId = [RHM_Comercial_RHM_ComercialDB createAttributeMetaData:attribute_counter:(SUPString)@"batchId":[SUPDataType forName:@"long"]];
 		...
 		RHM_Comercial_LocalKeyGeneratorPK_META_DATA.attributes = attributes;
 		
 		SUPAttributeMap * attributeMap = [SUPAttributeMap getInstance];
    	[attributeMap setAttributes:attributes];
    	RHM_Comercial_LocalKeyGeneratorPK_META_DATA.attributeMap = [attributeMap finishInit];
  
  		// Handle operations
 		int operation_counter = 0;	
 
  		SUPObjectList *operations = [SUPObjectList listWithCapacity:operation_counter];
 		RHM_Comercial_LocalKeyGeneratorPK_META_DATA.operations = operations;
 		
		SUPOperationMap *operationMap = [SUPOperationMap getInstance];
		[operationMap setOperations:operations];
		RHM_Comercial_LocalKeyGeneratorPK_META_DATA.operationMap = [operationMap finishInit];
	
     	_valid_ = YES;
	}
	return RHM_Comercial_LocalKeyGeneratorPK_META_DATA;
}

+ (RHM_Comercial_LocalKeyGeneratorPK*)getInstance
{
    RHM_Comercial_LocalKeyGeneratorPK* me = [[RHM_Comercial_LocalKeyGeneratorPK alloc] init];
    [me autorelease];
    return me;
}
